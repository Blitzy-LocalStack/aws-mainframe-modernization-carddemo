 ...
 * library-generated accessors, specifically Lombok. A generated accessor has no source of its own
 * on which the Javadoc required by line 15 of the Explainability rule can be written, so a
 * Lombok-built form of this type either fails the documentation gate outright or has to be exempted
 * from it, and no exemption exists: the audit configuration at
 * {@code config/checkstyle/checkstyle.xml} enables no comment-driven or annotation-driven
 * suppression filter, and its companion suppressions file reaches only generated sources and test
 * fixtures. A Java record reaches the same brevity by a route that keeps every member documentable,
...
public record ApiError(
     ...
     */
    public static final int CORRELATION_ID_WIDTH = 20;
     ...
     *     absent
     * @param path the failing request path, normalised to the empty string when absent
