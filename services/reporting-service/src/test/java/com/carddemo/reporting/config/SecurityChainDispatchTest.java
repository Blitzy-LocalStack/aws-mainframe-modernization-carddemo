 ...
 *
 * <p>⚠️ Refactoring Rationale: the condition pinned here was a real refusal, and this context
 * reaches it more often than most. Two of its eight operations produce
 * {@code application/octet-stream} and nothing else, so an {@code Accept} naming a renderable
 * type is refused before any handler runs -- and that refusal is exactly what the container
 * forwards to its error path. The forward keeps the original method and arrives with no
 * authentication, because the bearer-token filter skips an error dispatch, so it was judged by
 * {@code anyRequest().access(businessAccess())} with no principal and answered 401 in place of
 * the status this service had already decided.
 *
 ...
 * whose behaviour the dispatcher classes in {@code com.carddemo.reporting.api} already assert.
 *
...
class SecurityChainDispatchTest {
    ...
    private static final String BUSINESS_PATH = "/api/v1/reports/transaction-report/lines";
     ...
     * body, which is not a shape {@code openapi/reporting-api.yaml} declares for any operation.</p>
     *
