 ...
 * recorded beside that value in {@code services/card-service/src/main/resources/application-dev.yml}.
 *
 ...
 */
@DisplayName("the card service development profile")
final class DevProfileContractTest {
   ...
   *
   * <p>Assumptions: the widened timeout and the zero minimum are two halves of one decision and are asserted
   * together. A connection left open is what holds a zero-capacity cluster awake, so the pool must drain --
   * and a drained pool means the next caller may pay a cluster resume, which is why the wait is LENGTHENED
   * here rather than shortened. Asserting the ceiling alone would let either half be undone.
   */
  @Test
  @DisplayName("sizes the connection pool for the development load")
  void sizesTheConnectionPoolForTheDevelopmentLoad() {
    assertThat(dev.require("spring.datasource.hikari.maximum-pool-size")).isEqualTo("3");
  ...
  /**
   * The exposure list adds diagnostic endpoints while keeping the one both pollers read.
   *
   * <p>Assumptions: {@code health} is asserted PRESENT and that is the assertion that matters most. This
   * property binds as a set and a profile REPLACES an inherited collection rather than merging into it, so a
   * list naming only the added ids would remove the endpoint the load balancer target group and the container
   * health check both poll -- deregistering every task and failing container liveness at once.
   *
   * <p>Assumptions: the writable and memory-dumping ids are asserted ABSENT. {@code loggers} carries a write
   * operation, so whoever reaches it could raise the statement-parameter category this profile pins and make
   * a running task log card material; {@code heapdump} would leave a file carrying whatever the process held
   * in memory, which here includes the resolved database credential and any bearer token in flight.
   */
  @Test
  @DisplayName("publishes a read-only diagnostic surface that still carries health")
  void publishesADiagnosticSurfaceOfReadOnlyEndpointsOnly() {
    final List<String> exposed = dev.list("management.endpoints.web.exposure.include");

    assertThat(exposed).contains("health", "info", "prometheus");
    assertThat(exposed).contains("configprops", "env", "mappings");
   ...
   *
   * <p>Assumptions: the pin on the statement-parameter category is the load-bearing assertion in this context
   * above all others. This service stores the card verification value encrypted and returns it from no
   * endpoint, and the bound values of every statement it issues travel on that category -- so lowering it
   * would write into a log the one datum the whole masking and encryption design exists to keep out of one.
   */
  @Test
