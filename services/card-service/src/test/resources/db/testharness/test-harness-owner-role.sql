-- =============================================================================
-- services/card-service/src/test/resources/db/testharness/
--   test-harness-owner-role.sql
-- -----------------------------------------------------------------------------
-- Purpose:
--   Creates the NOLOGIN role `carddemo_card_owner` and grants it CREATE on the
...
--   owner-role callback issues `SET ROLE carddemo_card_owner` against a container
--   that has never heard of the role, and every context load fails with
--   `role "carddemo_card_owner" does not exist`.
--
...
--   The role `carddemo_card_owner` exists, holds NOLOGIN, and holds CREATE on the
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_roles WHERE rolname = 'carddemo_card_owner') THEN
        CREATE ROLE carddemo_card_owner NOLOGIN;
    END IF;
    EXECUTE format('GRANT CREATE ON DATABASE %I TO carddemo_card_owner', current_database());
END $$;
