...
CREATE TABLE card.cards (
    ...
    -- WHY : Assumptions: all three constraints below are named explicitly
    --       instead of taking a server-generated name, because each is
    --       quoted back to something that has to recognise it. A unique
    --       violation on the key and a check violation on the status are the
