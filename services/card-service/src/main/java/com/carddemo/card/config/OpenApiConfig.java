...
public class OpenApiConfig {
    ...
    public OpenAPI cardServiceOpenApi() {
        ...
        //       because the publishing library owns it outright. When springdoc.api-docs.version in
        //       application.yml selects the 3.1 form, the library overwrites both the version string
        //       and the specification flag on whatever document this method returns. A value set
        //       here would therefore be discarded without complaint, which is worse than absent: it
        //       would read as the effective version while having no effect. The committed contract
        //       and the generated document consequently agree on the specification MINOR version,
        //       which is what that key exists to hold, and the patch component is the library's to
        //       choose rather than this class's to assert.
        //
