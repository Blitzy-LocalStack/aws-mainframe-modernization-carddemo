...
--       no golden master, and CSUSR01Y.cpy L17-L23 contains only PIC X fields
--       with no monetary or encoded-decimal value. Assumptions: app/cpy files
--       provide provenance only; V1__auth.sql alone defines the load contract
--       consumed here.
--       (3) Assumptions: COCOM01Y.cpy L26-L28, not CSUSR01Y.cpy L22, authorizes
--       A for administrator and U for user. CSUSR01Y has no 88-level values, so
--       it establishes width but not the closed domain.
--       (4) Refactoring Rationale: the target declines parity with the legacy
...
--       remains registered.
--       (5) Assumptions: PostgreSQL blank-pads CHAR values but not VARCHAR
...
--       ambiguous.
--       (6) Assumptions: U means user in COCOM01Y.cpy L28 but update in the
...
--       (7) Assumptions: first and last names stay separate, following
...
--
-- Register of state that is deliberately absent from this fixture.
