 ...
 *
 * <p>Assumptions: no route here is administrator-only, and the omission is deliberate. The baseline
 * reaches account view and account update from the MAIN menu, {@code app/cbl/COMEN01C.cbl}, which both
 * user types reach, so restricting either operation to an administrator would REMOVE a capability an
 * ordinary user has today. Transformation rule T9 permits no behavioural change without a documented
 * divergence, and tightening a control is still a change.</p>
 *
