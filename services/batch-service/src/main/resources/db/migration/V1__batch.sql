...
CREATE TABLE batch.batch_run (
    ...
    -- WHY : Refactoring Rationale: this constraint pairs the lifecycle column
    --       with the exit-status TIER, and that pairing is the whole point of
    --       rewriting it. The earlier form asserted the two independently: it
    --       required only that a terminal row carry an end time, and left the
    --       tier check above to accept any admitted value in any state. Three
    --       rows that cannot exist were therefore storable, and each misdirects
    --       the redrive logic in a different way. COMPLETED with a code of 8 or
    --       more says the step both succeeded and hard-failed, so a redrive
    --       reading the status skips work that never finished. FAILED with 0
    --       says the opposite, so a redrive re-runs a step that did finish, and
    --       for the posting step re-running is not a wasted rerun -- it posts
    --       twice. FAILED with 4 is subtler and worse: 4 is the warn tier the
    --       chain is meant to CONTINUE through, so that row records a step the
    --       orchestrator would have continued past and the ledger says it must
    --       not. Pairing the two columns is the only way to exclude all three,
    --       because neither column is wrong on its own.
    -- WHY : Trade-offs: FAILED is the one state that admits a null exit status,
    --       and that asymmetry is deliberate rather than an oversight in the
    --       enumeration. A container killed by the platform -- out of memory, a
    --       task stopped by the scheduler, a node replaced under it -- reaches a
    --       terminal state without ever publishing an exit status, and that is
    --       precisely the failure an operator most needs recorded. Demanding a
    --       code from every terminal row would make the ledger unable to record
    --       it, and inventing one to satisfy the constraint would assert an
    --       outcome the process never published. COMPLETED admits no null, for
    --       the mirror reason: a step cannot be known to have completed cleanly
    --       unless it published the code that says so, so a null there is
    --       missing evidence rather than an honest state.
    -- WHY : Assumptions: the two admitted COMPLETED codes are the whole clean
    --       and warn set and nothing wider. 0 is clean; 4 is the soft warn whose
    --       single baseline origin is app/cbl/CBTRN02C.cbl line 229 to line 230,
    --       where a non-zero reject count moves 4 to RETURN-CODE, and which
    --       app/jcl/TRANBKP.jcl line 51 lets the chain continue through with
    --       COND=(4,LT). A reject correctly written is a completed step, not a
    --       failed one, which is why 4 belongs here and not with the failures.
    -- WHY : Assumptions: the COMPLETED branch writes "return_code IS NOT NULL
    --       AND return_code IN (0, 4)", and the first half of that pair is NOT
    --       redundant beside the second. It was measured: with the IN test alone,
    --       a COMPLETED row carrying a null code was ACCEPTED on PostgreSQL 17.10.
    --       An IN test against null evaluates to UNKNOWN rather than to false, an
    --       UNKNOWN conjunct makes the whole branch UNKNOWN, and a CHECK
    --       constraint treats UNKNOWN as satisfied -- so the one row this branch
    --       exists to exclude was the one row it let through. The explicit null
    --       test forces the branch to false and the disjunction to false. Null
    --       must be excluded here because COMPLETED asserts a step is known to
    --       have finished cleanly, and a null code is missing evidence for that
    --       claim rather than an honest state. The FAILED branch below needs no
    --       such guard, because it admits null deliberately and names it first.
    CONSTRAINT ck_batch_run_lifecycle
        CHECK ((status = 'STARTED'
               ...
               OR (status = 'COMPLETED'
                   ...
                   AND return_code IN (0, 4))
               OR (status = 'FAILED'
                   AND finished_at IS NOT NULL
                   AND (return_code IS NULL OR return_code >= 8))),

    -- WHY : Assumptions: a step cannot end before it began, and nothing else in
    --       this table asserts it. The lifecycle constraint above establishes
    --       that a terminal row HAS an end time; it says nothing about where
    --       that time sits relative to the start. An inverted pair passes every
    --       other check here and then yields a negative duration, which reads
    --       as a clock or a time-zone defect in whatever consumes it rather than
    --       as the bad row it is -- and the two columns are deliberately
    --       zone-less, so a consumer has no zone to blame it on and no way to
    --       repair it.
    -- WHY : Trade-offs: equality is admitted, so a step that begins and ends
    --       inside one microsecond is legal. The columns are TIMESTAMP(6) and a
    --       fast step genuinely can round to the same value at that resolution,
    --       so demanding strict inequality would reject a correct row. The cost
    --       is that a zero-length duration cannot be distinguished from a
    --       truncated one, which is the cheaper of the two mistakes.
    -- WHY : Assumptions: the comparison is written to pass while finished_at is
    --       null rather than relying on three-valued logic to do it silently. A
    --       bare comparison against null evaluates to unknown, which a CHECK
    --       treats as satisfied, so the null case would already pass -- but only
    --       as a side effect of SQL's null semantics. Stating it makes the
    --       running-step case a decision a reader can see instead of a property
    --       they have to derive.
    CONSTRAINT ck_batch_run_finished_after_started
...
COMMENT ON COLUMN batch.batch_run.status IS
    ...
    'cannot redefine stored rows. Domain enforced by ck_batch_run_status; the '
    'end time and exit status each state admits are enforced together by '
    'ck_batch_run_lifecycle, so STARTED carries neither, COMPLETED carries an '
    'end time with code 0 or 4, and FAILED carries an end time with code 8 or '
    'above or no code at all when the container was killed.';
...
COMMENT ON COLUMN batch.batch_run.finished_at IS
    ...
    'mid-flight. Never earlier than started_at, enforced by '
    'ck_batch_run_finished_after_started; equality is legal because a step can '
    'begin and end inside one microsecond at this resolution.';
...
COMMENT ON COLUMN batch.batch_run.return_code IS
    ...
    'publishing one -- which is why only FAILED admits a null code, never '
    'COMPLETED. The 4 tier originates at app/cbl/CBTRN02C.cbl:229-230. Which '
    'tier each state admits is enforced by ck_batch_run_lifecycle, so a row '
    'cannot claim to have completed and hard-failed at once.';
