 ...
 * disclosure-group record. One baseline program reads that record: {@code app/cbl/CBACT04C.cbl}
 * accrues interest and reaches the rate it multiplies by through this key and through nothing else.
 * Under transformation rule T1 the copybook, not the file description that repeats it, is the
 * normative source for every width and position below.</p>
 *
 * <h2>The layout</h2>
 *
 * <p>Offsets are zero-based, and they are the audit trail: every component traces to a byte range of
 * the record's 16-byte key.</p>
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Assumptions: the sixteen is confirmed from three independent places rather than derived once.
 * {@code app/cpy/CVTRA02Y.cpy} lines 6 to 8 declare the three components under the group item at line
 * 5; {@code app/cbl/CBACT04C.cbl} lines 78 to 81 repeat them at the same widths and line 50 names the
 * whole group as {@code RECORD KEY IS FD-DISCGRP-KEY}, so the reference system addresses the record by
 * one 16-byte value rather than by three lookups; and {@code app/jcl/DISCGRP.jcl} line 40 defines the
 * cluster with {@code KEYS(16 0)}, which balances only if the components are 10, 2 and 4 bytes wide.</p>
 *
 * <p>Assumptions: the component order is physical -- group, then type, then category -- and is
 * load-bearing rather than cosmetic, because a record's components are positional. The sibling
 * mapping {@code com.carddemo.batch.domain.DisclosureGroup.DisclosureGroupId} declares its three
 * members in the same order and the owning service's primary key is declared on
 * {@code (acct_group_id, tran_type_cd, tran_cat_cd)} to match, so this record, that mapping and the
 * physical index all agree. The rationale for not following the program's assignment sequence sits
 * beside the components themselves, where a reader with only those statements in view will meet it.</p>
 *
 ...
 * other two through unchanged. In {@code 1200-GET-INTEREST-RATE} at {@code app/cbl/CBACT04C.cbl}
 * lines 415 to 440, line 436 tests the file status for {@code '23'} and line 437 moves the literal
 * {@code 'DEFAULT'} into {@code FD-DIS-ACCT-GROUP-ID} and into no other field before line 438 retries
 * the read, with the type and category values still in place. The retry therefore asks for <i>the same
 * transaction type and category under the group named DEFAULT</i>. A wholesale default key would
 * resolve a different rate entirely, which is why {@link #withDefaultAccountGroupId()} derives from an
 * existing key and this type publishes no all-defaulted constant.</p>
 *
 * <p>Assumptions: the substituted value is ten characters and not the seven the literal is written
 * with. {@code 'DEFAULT'} is moved into a field declared {@code PIC X(10)}, and an alphanumeric move
 * left-justifies and space-fills, so the key the retry searches for is {@code DEFAULT} followed by
 * three spaces. {@link #DEFAULT_ACCOUNT_GROUP_ID} carries that padding as part of the value.</p>
 *
 * <p>Two outcomes of a lookup are decided by this key and recorded here for whoever implements the
 * lookup; neither behaviour lives in this type, the result being {@code InterestRateLookup} and the
 * read belonging to a repository. A missing group record is an ordinary, expected outcome -- line 422
 * accepts a status of {@code '00'} or {@code '23'} as equally non-fatal and the not-found case is the
 * fallback path. A missing DEFAULT record is a hard failure -- the retry read at line 444 carries no
 * {@code INVALID KEY} clause and the status test at line 446 accepts only {@code '00'}, so a key still
 * absent after the substitution abends. There is no third fallback and no rate compiled into the
 * program, so the fallback does not resolve to zero interest, it stops the run.</p>
 *
 * <p>Assumptions: because the substitution replaces one component of three, the seed requirement is
 * not one row. A distinct DEFAULT-group row is needed for every transaction type and category pair an
 * account can present, and those rows are reference data seeded by {@code reference-service}. This
 * module neither creates nor seeds them.</p>
 *
 ...
 * because both are declared {@code PIC X}, so each is a code rather than a quantity and its declared
 * width is part of the key contract {@code app/jcl/DISCGRP.jcl} line 40 counts in bytes. Trailing
 * blanks in either are significant characters occupying declared positions, not padding this type may
 * discard.</p>
 *
 * <p>Assumptions: the transaction category code is a number, because
 * {@code DIS-TRAN-CAT-CD PIC 9(04)} is numeric-display -- a numeric picture right-justifies and
 * zero-fills, so the value 5 occupies the record as {@code 0005} and never as {@code 5} followed by
 * blanks. An {@code int} carries every value the four digits admit and
 * {@link #transactionCategoryCodeField()} supplies the width. Splitting the concern that way keeps
 * {@code 5} and {@code 0005} from being two keys, which is the hazard a text type would carry here: a
 * caller omitting a leading zero would build a different key and the difference would surface only as
 * a lookup that matched nothing. The category member of the sibling mapping is a fixed-character
 * string instead, because it has to match a {@code CHAR(4)} column, and
 * {@link #transactionCategoryCodeField()} is the only place that conversion is expressed.</p>
 *
 * <h2>Validation, equality and what strictness costs</h2>
 *
 * <p>Assumptions: value equality is literal -- all three components compared, none trimmed,
 * normalised or case-folded -- because the column behind this key is {@code CHAR(10)} and a
 * fixed-character column treats trailing blanks as insignificant in both directions. Trimming here
 * would import that leniency into Java, where it does not hold, so a padded key and a bare one would
 * become interchangeable inside a hash-based collection while still rendering as two different 16-byte
 * keys. The generated {@code equals}, {@code hashCode} and {@code toString} are therefore kept: the
 * generated rendering wraps the components in their names, which makes it unmistakable for the
 * 16-byte wire form {@link #fixedWidthKey()} produces.</p>
 *
 * <p>Trade-offs: the compact constructor requires the account group identifier to be <b>exactly</b>
 * ten characters and the transaction type code exactly two, rather than accepting them short and
 * padding on the way in. The cost is real: every construction site has to reach the declared width
 * first, and a value read back out of a fixed-character column arrives short, which is why
 * {@link #ofBlankPaddedAccountGroupId(String, String, int)} exists as the one named place that
 * conversion happens. What strictness buys is that a key has exactly one canonical form, which is what
 * makes {@link #isDefaultAccountGroup()} answerable correctly -- under accept-and-pad, a key built
 * from a blank-stripped {@code DEFAULT} would be unequal to the padded form and would report that the
 * fallback had not fired when it had. Strictness makes that wrong answer unrepresentable rather than
 * merely documented.</p>
 *
...
public record DisclosureGroupKey(
    ...
    // Assumptions: the three components above are declared in the PHYSICAL order of the
    ...
    // Alternatives Considered: declaring them in the order app/cbl/CBACT04C.cbl assigns them
    ...
    // Assumptions: the constant below is TEN characters and not the seven its baseline
    ...
    // Trade-offs: the padded form is what this type publishes even though the column behind
     ...
     * component, whose alphanumeric picture pads on the right, and the two are held as separate
     * literals so neither is used for the other. A suffix of this literal supplies precisely the
     * leading zeros a value of one, two or three digits is short by.</p>
     *
     * <p>Alternatives Considered: a formatter with a zero-padded numeric conversion, rejected on a
     * concrete ground rather than a stylistic one -- that conversion localises its digits, so under a
     * default locale whose decimal digits are not ASCII it would emit characters the 16-byte key has
     * no room for. {@code Integer.toString} is specified to emit ASCII digits at radix ten regardless
     * of locale, which is why it is paired with this literal instead.</p>
     */
    ...
    public DisclosureGroupKey {
        // Assumptions: one exception type covers an absent component, a mis-sized one and an
        ...
        // Trade-offs: the width is required EXACTLY rather than accepted short and padded
        ...
        // Assumptions: the width is measured on the value as supplied, with no whitespace
        ...
        // Assumptions: the type code is required at exactly two characters and is not padded
        ...
        // Assumptions: the range is what the four-digit unsigned picture at
    ...
    public static DisclosureGroupKey ofBlankPaddedAccountGroupId(
        ...
        // Assumptions: a value ALREADY at the declared width is passed through untouched, and
        ...
        // Assumptions: the padding is a SUFFIX of a blank literal of the declared width, so
    ...
    public String accountGroupId() {
        // Assumptions: this accessor is written out rather than left implicit purely so that
    ...
    public String transactionTypeCode() {
        // Assumptions: written out for the same reason as the accessor above -- the contract
    ...
    public int transactionCategoryCode() {
        // Assumptions: written out so that the split between this method and the field
    ...
    public DisclosureGroupKey withDefaultAccountGroupId() {
        // Assumptions: ONE component of three is replaced. Replacing all three -- which a
    ...
    public boolean isDefaultAccountGroup() {
        // Assumptions: the constant is the receiver of the comparison rather than the
    ...
    public String transactionCategoryCodeField() {
        // Assumptions: Integer.toString is specified to emit ASCII digits at radix ten
        ...
        // Assumptions: the substring index is safe without a guard because the compact
    ...
    public String fixedWidthKey() {
        // Assumptions: the two text components are concatenated as held, with nothing trimmed
