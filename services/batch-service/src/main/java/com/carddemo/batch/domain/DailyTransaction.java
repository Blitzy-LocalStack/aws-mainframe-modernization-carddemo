...
public class DailyTransaction {
    ...
    //       digits than the record carries and the value would stop matching the bytes it came
     ...
     *
     * <p>Refactoring Rationale: a surrogate ingestion sequence carries the identity rather than
     * {@code transactionId}, because the feed's own identifier is not unique. Mapping the identifier as
     * the identity would make two rows repeating it share one entity identity, and a documented caveat
     * that a consumer "must treat a scan of this feed as a stream" cannot correct that: the chunked
     * reader scanning this feed orders and resumes on its identity column, so a duplicate identifier
     * straddling a chunk boundary would be skipped outright. A caveat cannot correct a cursor, which is
     * why the fix is a column and not a sentence.</p>
     */
    ...
    // WHY : Assumptions: this column is NOT the entity identity, which is what makes a repeated
    ...
    //       or two and would stop matching the bytes it was read from.
    ...
    // WHY : Refactoring Rationale: NOT NULL rather than nullable.
    //       `DALYTRAN-AMT PIC S9(09)V99` declares no absent state, and neither
    ...
    public DailyTransaction(String transactionId, String typeCd, String categoryCd, String source,
        ...
        //       verbatim. Verbatim assignment would admit three defects: a null into a column declared
        //       NOT NULL, a scale other than two that the driver would coerce, and a ten-integer-digit
        //       magnitude that Money.of admits for the widest reference picture and that NUMERIC(11,2)
        //       then rejects at the database with no field name in the failure. Bounding at the picture
        //       rejects all three where the value entered the row.
     ...
     *
     * <p>Refactoring Rationale: the comparison is on the ingestion sequence and NOT on the transaction
     * identifier. The table asserts no uniqueness over the identifier, so comparing it would make two
     * rows of a feed that legitimately repeated one compare equal, and recording that as a caveat does
     * not prevent the loss it describes. The sequence the owning migration declares is unique per row,
     * so the hazard is removed rather than documented.
     *
     ...
     * must agree -- hashing a value the equality does not consult would let two instances the
     ...
     *
     * <p>Refactoring Rationale: the transaction identifier is NOT hashed, even though hashing it would
     * pair with an equality that compared it, and even though this read-only type exposes no mutator for
     * it. Immutability is an argument about STABILITY and says nothing about UNIQUENESS, which is the
     * property a hash key actually needs: two distinct arrivals sharing an identifier would hash alike
     * and compare equal, so a set would discard one. A hash derived from the SEQUENCE carries the
     * opposite hazard: an instance hashed before the database assigned its value would sit in a bucket
     * keyed by the old hash and become unfindable in a collection that still held it. A constant is the
     * only choice that avoids both. </p>
     *
     ...
     * characterise it.
     *
     * <p>Refactoring Rationale: the ingestion sequence LEADS the rendering. It is this type's mapped
     * identity, so a line omitting it would name only a transaction identifier that two physical rows
     * can share -- precisely the ambiguity the identity sits on the sequence to remove. A diagnostic
     * that cannot say WHICH occurrence it concerns fails in the one case where a reader most needs
     * it. </p>
     *
