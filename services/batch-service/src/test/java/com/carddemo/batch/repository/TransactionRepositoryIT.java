 ...
 * authority for all of them. Tier one's interest test owns ALL interest arithmetic, including that the
 * divide TRUNCATES rather than rounding half up -- no {@code ROUNDED} phrase appears in
 * {@code app/cbl/CBACT04C.cbl} or anywhere else under {@code app/cbl} -- so no case here asserts any
 * rounding behaviour on that path. Tier two's job tests own the graded return-code tier, the inversion
