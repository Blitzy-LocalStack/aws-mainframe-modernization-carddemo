...
/**
 * Records, finds and retires the reply already produced for one date-conversion request.
 *
 * <p><b>Purpose.</b> The asynchronous date-conversion exchange transcribed from
 * {@code app/app-vsam-mq/cbl/CODATE01.cbl} answers a request by sending a reply and then returning, and
 ...
 * killed there, a container cycled there or an acknowledgement lost there leaves the request visible
 * again and hands it to another delivery. This class is the durable side of the remedy: the composed
 * reply is recorded under the identity the QUEUE SERVICE assigned the delivery and committed BEFORE it
 * is sent, so a redelivery can discover the answer that was already produced and re-send those bytes.</p>
 *
 * <p>Assumptions: for THIS exchange the recorded bytes matter more than the suppressed duplicate, and
 * that is why the payload is stored rather than merely a flag. The reply body is the system date and
 * time, asked of the clock at the moment the reply is composed --
 * {@code app/app-vsam-mq/cbl/CODATE01.cbl} asks at physical lines 343 to 345 and formats at 347 to 353
 * -- so a redelivery that recomposed its answer would produce a LATER timestamp, and a requester
 * pairing on one correlation identifier would hold two replies that disagree with no way to tell which
 * the exchange regarded as its answer. Re-sending the recorded bytes makes the first answer the only
 * answer, whichever delivery gives it.</p>
 *
 ...
 * seven types in this package are. Rejected because the claim is not expressible through one, as above,
 * and because a mapped entity would let any member of this module read or write the ledger through the
 * persistence context -- including flushing a stale copy over a row a concurrent delivery had already
 * advanced. Three named statements over one table claim nothing else and offer nothing else. The
 * precedent is the identically shaped ledger in {@code account-service}, and it is followed rather than
 * re-argued so the two exchanges cannot acquire two idempotency disciplines.</p>
 *
 ...
 * would both find nothing and both proceed; the primary key does the same work at the moment of
 * insertion, which is the only moment at which it can be done.</p>
 *
 * <p>Trade-offs: this narrows divergence to a single crash window rather than eliminating duplication. A
 * task that dies after the send and before {@link #markSent} leaves the row PENDING, so the redelivery
 * re-sends the recorded reply and the requester receives two copies of one answer -- identical copies,
 * since the payload is stored verbatim rather than recomposed, and each carrying the same two echoed
 * identities. Closing that window entirely would require the send and the mark to commit together across
 * two resource managers, which is the two-phase commit AAP section 0.7.6 records as eliminated. What is
 * bought is that a DIFFERING answer is no longer reachable at all, and that duplication is one specific
 * failure instead of every redelivery.</p>
 *
 * <p>Parameters, return values, exceptions or errors: this class declares no constructor a caller
 * supplies arguments to and yields no value of its own; each member carries its own at-clauses. The
 * inapplicability is stated rather than passed over, because the Explainability rule forbids a docstring
 * that omits parameters or return values and a reader must be able to tell a declared inapplicability
 * from an oversight.</p>
 */
@Repository
public class InquiryReplyLedger {
     ...
     * constraint in {@code V3__reference_inquiry_reply_ledger.sql} admits. A second spelling of it in
     * either place would be a state the database refuses.</p>
     */
    ...
    private static final String CLAIM_REPLY = """
            insert into reference.inquiry_reply_ledger (
    ...
    private static final String FIND_CLAIM = """
            ...
            from reference.inquiry_reply_ledger
    ...
    private static final String MARK_SENT = """
            update reference.inquiry_reply_ledger
     ...
     * the arrangement the sibling ledger in {@code account-service} uses. The proxy resolves the
     * transaction-bound entity manager per call, so this bean holds no connection and carries no
     * per-request state.</p>
     */
    @PersistenceContext
     ...
     * @param payload the framed reply, stored verbatim so a re-send sends the same bytes rather than a
     *     later timestamp; must not be {@code null}
     ...
     * @param correlationId the correlation identity to echo, or {@code null} if none was carried
     * @param messageId the message identity to echo, or {@code null} if none was carried
     */
