...
CREATE TABLE reference.transaction_types (
    -- WHY : Assumptions: CHAR(2) rather than VARCHAR(2) because the width is
    ...
    -- WHY : Assumptions: VARCHAR(50) rather than CHAR(50) because the trailing
...
CREATE TABLE reference.transaction_categories (
    ...
    -- WHY : Assumptions: CHAR(4), even though app/cpy/CVTRA04Y.cpy L7 declares
    ...
    -- WHY : Assumptions: VARCHAR(50) for the same reason as the description on
    ...
    -- WHY : Refactoring Rationale: RESTRICT, and never CASCADE, SET NULL or NO
...
-- WHY : Trade-offs: no index is created on transaction_categories.type_cd for
...
CREATE TABLE reference.disclosure_groups (
    -- WHY : Assumptions: CHAR(10) exactly, because the space padding is part of
    ...
    -- WHY : Assumptions: CHAR(4) for the same six reasons set out at
    ...
    -- WHY : Alternatives Considered: NUMERIC(6,2) is the exact image of
...
-- WHY : Alternatives Considered: no foreign key runs from disclosure_groups to
...
CREATE TABLE reference.us_phone_area_codes (
    ...
    -- WHY : Alternatives Considered: one table with a classification column,
    ...
    -- WHY : Assumptions: two values and no more, mirroring the partition above.
...
CREATE TABLE reference.us_states (
    -- WHY : Assumptions: CHAR(2) because the baseline compares a two-byte field
...
CREATE TABLE reference.us_state_zip_prefixes (
    -- WHY : Assumptions: CHAR(4) holding the state code and the two leading
