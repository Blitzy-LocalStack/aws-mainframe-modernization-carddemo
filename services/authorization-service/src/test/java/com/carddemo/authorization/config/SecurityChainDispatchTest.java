 ...
 * against the decision object {@link SecurityConfig#businessAccess()} or
 * {@link SecurityConfig#fraudAccess()} returns, because a decision object is addressable without a
 * servlet container and keeps the assertion uniform across the eight contexts. The rule this class covers
 * is not expressible that way at all: it matches a DISPATCHER TYPE rather than a path or an authority, so
 * the only thing that can witness it is a dispatch. That is why this one class assembles a web context and
 ...
 *
 * <p>Measured: with the dispatcher-type permit removed from the chain, exactly the two ERROR-dispatch
 * cases here fail -- {@link #theChainIsInstalled()} and
 * {@link #aDirectRequestToTheErrorPathIsStillRefused()} keep passing, which is what shows the four cases
 * divide the rule from the catch-all it sits in front of rather than measuring one condition four times.
 *
 ...
 * handler whose behaviour two sibling classes already assert.
 *
...
class SecurityChainDispatchTest {
     ...
     * error body, which is not a shape {@code openapi/authorization-api.yaml} declares for any
     * operation.</p>
     *
