...
## 5. The remaining fourteen resources, and what asserts each

Assumptions: sections 1 to 3 describe eighteen of the thirty-three resources in this directory. The
fourteen below arrived with the detail-segment, decline-reason and fraud-domain work and are recorded
here so the inventory in
`AuthorizationFixtureContractTest` has a documented counterpart for every name it lists — that class
enrols all thirty-three as a closed set, and a name it enumerates with nothing written about it
