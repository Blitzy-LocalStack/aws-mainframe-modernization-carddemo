 ...
 *
 * <p>Two of this service's capabilities are jobs rather than requests: loading the pending-authorization
 * extracts, and purging authorizations that have expired. Each runs for as long as its input takes, ends
 * with a status a scheduler branches on, and must run exactly once per invocation. This package holds the
 * three things that make that possible -- the {@link com.carddemo.authorization.task.AuthorizationTask}
 * contract, the two beans that implement it, and the
 * {@link com.carddemo.authorization.task.MaintenanceTaskRunner} process entry point that resolves one by
 * name and translates its outcome into an exit status.</p>
 *
 * <h2>Why it exists at all</h2>
 *
 * <p>Refactoring Rationale: it was added because neither job had a production invocation path. The service
 * package's own documentation described both as orchestrator-invoked, and the orchestrator had no state for
 * either -- no schedule, no controller, no runner and no state in the batch state machine -- so each was
