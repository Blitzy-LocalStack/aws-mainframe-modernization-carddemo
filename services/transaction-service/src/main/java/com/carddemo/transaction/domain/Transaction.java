...
public class Transaction {
    ...
    /**
     * Integer digit positions the amount's picture declares, at code app/cpy/CVTRA05Y.cpy:10.
     *
     ...
     * rescaling would hide a caller that had lost a fraction of a cent upstream.</p>
     *
     ...
     * earlier revision did on the reasoning that "rounding a monetary value is a decision with an
     * owner and this type is not it". Money IS that owner, and delegating to it is not the same as
     * making the decision here: what verbatim assignment actually admitted was a null into a column
     * declared {@code NOT NULL}, a scale other than two that the driver would then coerce without
     * telling anyone, and a ten-integer-digit magnitude that is legal for the widest reference money
     * picture and illegal for this nine-digit one -- which reached {@code NUMERIC(11,2)} and raised a
     * numeric-field-overflow at the database with no field name in it. All three are now refused at
     * the assignment that introduced them.</p>
     *
     * @param tranAmt the amount to assign; must not be {@code null}, is reduced to two decimal
     *     places under the general money contract, and must fit nine integer digits
