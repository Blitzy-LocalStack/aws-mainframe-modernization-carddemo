...
/**
 * The outcome of the migrated bill-payment submission: the account that was paid, the balance that
 * payment settled, the key of the transaction written for it, and the sentence accompanying the
 * turn.
 *
 * <p><b>Purpose.</b> This is the outbound body of the bill-payment endpoint, produced by
 * {@code com.carddemo.transaction.service} and returned by a controller in
 * {@code com.carddemo.transaction.api}. It holds no logic and reaches nothing: each of its four
 * components is a value the reference screen reports or the reference program persists. The
 * reference COBOL is the specification, so this shape encodes the field set, the widths and the
 * scale that specification already states rather than redefining them.
 *
 * <p><b>Parameters, return values, exceptions or errors.</b> The four components are documented
 ...
 * credited.
 *
 * <h2>The balance reported is the balance BEFORE the payment</h2>
 *
 ...
 * the balance and the amount paid are one number rather than two. Line 233 writes the transaction;
 * only afterwards does line 234 subtract the amount from the balance, and line 235 rewrite the
 * account. Line 242 sends the screen. No statement between line 194 and line 242 repopulates that
 * map field, so the figure the operator reads is the balance as it stood before the payment.
 *
 * <p>Trade-offs: there is exactly one balance component here and no second one reporting the
 * balance after the payment. Lines 194, 234 and 242 together are the reason: the value line 234
 * computes is stored by line 235 but never reaches the field line 194 filled, so a post-payment
 * figure is not something this reference screen displays. The compromise accepted is that a client
 * wanting the resulting balance re-reads the account through the context that owns it. Adding a
 * second balance component instead would surface a value this screen never shows, which is a change
 * in observable behaviour rather than in structure, and transformation rule T9 permits the second
 * and forbids the first.
 *
 ...
 *
 * <p>Assumptions: a submission that moved no money still answers with this shape, which is why the
 * message component has to be able to speak on its own. Lines 198 and 199 of that program test the
 ...
 * line 203, writing nothing. The balance component then carries the figure line 193 had already
 * moved, and there is no transaction key to report.
 *
 ...
 * included. Two of the four travel in this component, being the turns that answer rather than
 * reject: {@code 'Confirm to make a bill payment...'} at line 237, the withheld-confirmation turn,
 * and {@code 'You have nothing to pay...'} at line 201, the turn whose balance is not positive. The
 * other two are complaints about one field and travel in the per-field error array instead:
 * {@code 'Invalid value. Valid values are (Y/N)...'} at line 187, and
 * {@code 'Acct ID can NOT be empty...'} at line 161, whose capitalisation is the program's own and
 * which {@code BillPaymentRequest} already carries onto its not-blank constraint. The program's
 ...
 *
 * <p>Assumptions: the ten values the reference program hard-codes onto the transaction it writes
 * have no component here, and naming them is what shows the omission to be deliberate. Lines 220 to
 * 229 move a type code of {@code '02'}, a category of {@code 2}, a source of {@code 'POS TERM'}, a
 * description of {@code 'BILL PAYMENT - ONLINE'}, the account balance as the amount, the
 * cross-referenced card number, a merchant identifier of {@code 999999999}, a merchant name of
 * {@code 'BILL PAYMENT'} and {@code 'N/A'} for both the merchant city and the merchant postal code.
 * Lines 230 to 232 then move one timestamp into both {@code TRAN-ORIG-TS} and {@code TRAN-PROC-TS}
 * in a single statement. Every one of them is a constant of the writing program rather than an
 * outcome of the request, and each is already retrievable at the detail endpoint under the key this
 * response returns.
 *
 ...
 *
 * <p>Trade-offs: there is no echo of the written record on this response. A client wanting the
 * transaction as stored reads the detail endpoint of this context, which returns it at record
 * width. What is bought is that this body reports the outcome of the operation and nothing else;
 * the cost is one further call for a client that wants the whole record, and the key that makes
 * that call possible is the third component below.
 *
 * <p>Assumptions: no card-number component appears here and none may be added. Line 225 takes the
 * card number from the cross-reference record read at line 211, so it is a value the payment looked
 * up rather than an outcome it produced, and the reference map declares no card field at all. Where
 * a card number is carried elsewhere in this package it is masked to its last four digits and the
 * masking is applied in {@code com.carddemo.transaction.mapper}, never in a transfer object.
 * Nothing here carries a card verification value, a credential, a connection string or an endpoint.
 *
 ...
 * classes.
 *
 * <h2>Four components, where the published contract describes seven</h2>
 *
 * <p>Trade-offs: this record implements the four-component shape the migration plan assigns it,
 * while {@code services/transaction-service/src/main/resources/openapi/transaction-api.yaml}
 * describes a wider shape under the same name. Alongside the account identifier, the balance, the
 * transaction key and the message, that document adds a masked card number, a separate amount-paid
 * member and a boolean flag recording that money moved, and it describes its own balance member as
 * the figure line 234 computes rather than the figure line 194 displays. The difference is recorded
 * here rather than left for a reader to discover, because an omission in a record is invisible: the
 * four names declared below are all names that document also uses, so the two agree on naming and
 * differ only in breadth and in which side of line 234 the balance is read from. What the narrower
 * shape buys is that every component traces to a value the reference screen reports or the
 * reference program persists at the moment it is read, which the pre-payment proof above
 * establishes; what it gives up is that a client reading that document finds three members this
 * body does not send. Reconciling the two is an edit to that document and is not made from here.
 *
 ...
 * that produced it. Here the component's meaning is fixed by the documentation on it and does not
 * vary by turn.
 *
...
public record BillPaymentResponse(
    ...
    //       fixes it: COBIL00C.cbl line 194 fills the screen field, line 224 reuses the same
    //       untouched value as the transaction amount, line 233 writes, and only line 234
    //       subtracts. Nothing refills that field before line 242 sends it.
    ...
    //       screen never displays, which changes observable behaviour rather than structure.
