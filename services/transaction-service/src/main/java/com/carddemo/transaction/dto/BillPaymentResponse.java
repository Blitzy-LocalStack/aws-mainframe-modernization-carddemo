 ...
 * @param paid always {@code true} on this shape, discriminating a posted payment from
 *     {@link BillPaymentPreview}, which the same operation publishes on 200 with the value
 *     fixed to {@code false};
...
public record BillPaymentResponse(
    ...
    //       PAIR -- BillPaymentPreview carries the same member fixed to false, so a body
    //       read without its status code still says which of the two it is.
    boolean paid,
    ...
    /**
     * Renders this response WITHOUT the account it settled or the balance it paid.
     *
     * <p>Purpose. The account identifier and the pre-payment balance are both prohibited from a
     * diagnostic rendering by {@code docs/architecture/observability.md} L1093 to L1112, the first by
     * name and the second as a monetary amount belonging to an identified account. This shape is
     * produced only on the confirmed path, so the compiler-generated rendering would have printed the
     * exact sum moved off a named account every time a payment succeeded.</p>
     *
     * <p>Assumptions: the transaction identifier is KEPT while the account identifier is omitted, and
     * the asymmetry is that rule's third clause rather than an inconsistency. A transaction identifier
     * is named there as identity that discloses nothing -- it is generated by this service, names one
     * ledger row and no party -- and it is what makes this rendering worth emitting, because a posted
     * payment can be traced to the row it wrote without naming whose it was.</p>
     *
     * <p>Trade-offs: an operator reading logs alone can no longer see the amount a payment moved and
     * must read the ledger row to learn it. That cost is accepted: the row is reachable from the
     * identifier this rendering does carry, so the information is one query away rather than lost.</p>
     *
     * @return a rendering carrying the transaction identifier, the paid discriminator and the return
     *     message, with the account identifier and the balance omitted entirely; never {@code null}
     */
    @Override
    public String toString() {
        return "BillPaymentResponse[transactionId=" + this.transactionId
                + ", paid=" + this.paid
