...
--     application.yml sets spring.flyway.create-schemas to false at its L426
...
--       connection, at application.yml L291 `connection-init-sql: SET
...
--       L543-L552 records the deliberate decision to set no schema search order
...
--
-- WHY : Alternatives Considered: batch-service writes these tables under a
--       second login holding narrowly scoped cross-schema write privileges on
--       `ledger.*` and `account.*`, established in
--       data-migration/sql/V0__schemas_and_roles.sql section 4. A reader who
--       does not know that sees an apparent one-schema-per-service violation
--       here, so the reason is recorded beside the tables it applies to. In
--       app/cbl/CBTRN02C.cbl the paragraph 2000-POST-TRANSACTION at L424
--       performs 2700-UPDATE-TCATBAL at L440, 2800-UPDATE-ACCOUNT-REC at L441
--       and 2900-WRITE-TRANSACTION-FILE at L442, closing at L444: two of those
--       three records land in `ledger` and one in `account`, inside one unit of
--       work. A transactional-outbox-plus-compensating-reversal design was
--       considered and rejected, because it would introduce observable
--       partial-posting states that do not exist in the baseline, which would
--       break golden-master parity outright. A saga across separate databases
--       fails for the same concrete reason. Keeping one database and narrowing
--       the second login's privileges is what holds the posting commit atomic,
--       so any change to this file is a change to that unit of work.
--
-- WHY : Trade-offs: NOT NULL is asserted on primary-key columns, which
--       PostgreSQL requires in any case, and on transactions.proc_ts, and
--       nowhere else. The restraint is deliberate and it costs declared
--       strictness. A blank fixed-width field decodes to NULL at the load
--       boundary, and that is measured rather than hypothetical: in
--       app/data/ASCII/dailytran.txt the 26 bytes at 305-330 are blank on all
--       300 of 300 records. So blank fields do occur in the extracts these
--       tables are loaded from, and asserting NOT NULL more widely would refuse
--       a load the baseline itself accepts. The accepted cost is that column
--       presence beyond the keys is enforced by the writing service rather than
--       by the schema. The one exception is justified where it is declared.
--
-- WHY : Refactoring Rationale: the baseline file these tables replace is
--       defined at app/csd/CARDDEMO.CSD L76 as `DEFINE FILE(TRANSACT)` over
--       `DSNAME(AWS.M2.CARDDEMO.TRANSACT.VSAM.KSDS)` at L77, and it carries
--       `JOURNAL(NO)` at the end of L82 and `RECOVERY(NONE)` at L84. It
--       therefore had neither journalling nor recoverability of its own.
--       Nothing in this file reproduces that posture, and nothing in this file
--       configures the replacement either: encryption at rest and automated
--       backups are cluster properties carried by
--       infra/modules/aurora-postgresql, not table properties. This is recorded
--       here so the absence of any storage or recovery clause below reads as a
...
CREATE TABLE ledger.transactions (
    ...
    -- WHY : Assumptions: CHAR(16) rather than an integer, for a field whose
    ...
    -- WHY : Assumptions: a two-character code, so CHAR(2) by the fixed-code
    ...
    -- WHY : Assumptions: CHAR(4) although app/cpy/CVTRA05Y.cpy L7 declares
    ...
    -- WHY : Assumptions: CHAR(10) keeps the blank-padded comparison semantics
    ...
    -- WHY : Assumptions: VARCHAR(100), not CHAR, because this is descriptive
    ...
    -- WHY : Assumptions: exact fixed point at precision 11, scale 2, because
    ...
    -- WHY : Assumptions: an identifier, so BIGINT by the numeric-identifier
    ...
    -- WHY : Assumptions: descriptive, so VARCHAR by the same reasoning as the
    ...
    -- WHY : Assumptions: descriptive, VARCHAR for the reason above; longest
    --       trimmed value measured is 19 of 50. Bytes 203-252.
    merchant_city   VARCHAR(50),

    -- WHY : Assumptions: CHAR(10) rather than VARCHAR, because a postal code is
    ...
    -- WHY : Assumptions: CHAR(16) and never a numeric type.
    ...
    --       leading zero -- `0927987108636232`, `0500024453765740` and
    --       `0923877193247330` among them -- so an integer column would
    --       silently shorten them to 15 digits and break every lookup on this
    --       value. app/jcl/TRANREPT.jcl L41 types the same field
    ...
    -- WHY : Assumptions: TIMESTAMP(6) matches `PIC X(26)` exactly. The
    ...
    -- WHY : Assumptions: the one NOT NULL beyond the primary key in this file,
    ...
    -- WHY : Assumptions: a single-column key on the transaction identifier,
...
-- WHAT: the by-card access path over the posted transaction master.
-- WHY : Assumptions: the baseline reaches these records in card order without
...
-- WHAT: the by-processing-timestamp access path over the posted transaction
--       master.
-- WHY : Refactoring Rationale: this index is the surviving half of the batch
...
--
-- WHY : Trade-offs: the column list is repeated rather than shared. PostgreSQL
...
CREATE TABLE ledger.daily_transactions (
    ...
    -- WHY : Assumptions: alphanumeric picture with significant leading zeros,
    ...
    -- WHY : Assumptions: fixed-width code, bytes 17-18; argued at
    --       ledger.transactions.type_cd.
    type_cd         CHAR(2),

    -- WHY : Assumptions: a code rather than a quantity despite its `9(04)`
    --       picture, bytes 19-22; argued at ledger.transactions.category_cd.
    category_cd     CHAR(4),

    -- WHY : Assumptions: closed label set compared blank-padded, bytes 23-32;
    --       argued at ledger.transactions.source.
    source          CHAR(10),

    -- WHY : Assumptions: descriptive text whose trailing blanks are padding,
    --       bytes 33-132; argued at ledger.transactions.description.
    description     VARCHAR(100),

    -- WHY : Assumptions: exact fixed point from `PIC S9(09)V99`, bytes 133-143;
    ...
    -- WHY : Assumptions: numeric identifier with no significant leading zero,
    --       bytes 144-152; argued at ledger.transactions.merchant_id.
    merchant_id     BIGINT,

    -- WHY : Assumptions: descriptive, bytes 153-202; argued at
    --       ledger.transactions.merchant_name.
    merchant_name   VARCHAR(50),

    -- WHY : Assumptions: descriptive, bytes 203-252; argued at
    --       ledger.transactions.merchant_city.
    merchant_city   VARCHAR(50),

    -- WHY : Assumptions: postal code with significant leading zeros, bytes
    --       253-262; argued at ledger.transactions.merchant_zip.
    merchant_zip    CHAR(10),

    -- WHY : Assumptions: alphanumeric, 30 of 300 measured values carry a
    ...
    -- WHY : Assumptions: `PIC X(26)` to microsecond precision, bytes 279-304;
    ...
    -- WHY : Assumptions: nullable here while ledger.transactions.proc_ts is NOT
...
-- WHY : Assumptions: this table has no primary key and no index, and that is
...
CREATE TABLE ledger.transaction_rejects (
    ...
    -- WHY : Trade-offs: the rejected record is retained verbatim as one fixed
    --       350-byte value and is deliberately NOT decomposed into the thirteen
    --       columns its layout would yield. Decomposing it would lose the very
    --       thing it is kept for: the bytes that caused the reject. A record
    --       rejected as reason 100 has a card number that resolved to nothing,
    --       and a reason-102 record may hold an amount no validated column
    --       should accept, so parsing it into typed columns would either fail
    --       on the malformed input or normalise away the evidence, and a
    --       re-drive would then replay something other than what arrived. The
    --       program treats it as opaque for the same reason: CBTRN02C L447 is
    --       `MOVE DALYTRAN-RECORD TO REJECT-TRAN-DATA`, a wholesale copy with
    --       no field-level handling. CHAR rather than VARCHAR because the
    --       existing golden-master comparison reads these records back at 350
    --       bytes including trailing blanks, and only the fixed-width type
    --       guarantees that width no matter what a writer supplies: a short
    --       write is padded out to 350 here, whereas VARCHAR(350) would
    --       faithfully store the short value and silently shorten the compared
    --       record. The accepted cost is that reading a single field back out
    --       of this column requires a substring at a documented offset, and one
    --       retrieval detail is worth knowing before it looks like data loss:
    --       the SQL length() function and an explicit cast to text both report
    --       the value with trailing blanks removed, so length() over a 350-byte
    --       record returns the trimmed figure while octet_length() returns 350.
    --       The padding is present in storage and in what a client reads; it is
    --       those two operations that hide it, so a 430-byte reconstruction
    --       must pad the cast result back to 350 before appending the trailer.
    raw_record   CHAR(350),

    -- WHY : Assumptions: SMALLINT is the narrowest exact integer type covering
    ...
    -- WHY : Assumptions: descriptive text, so VARCHAR, holding the declared
...
-- WHY : Assumptions: no primary key, no unique constraint and no index, because
...
--
-- WHY : Assumptions: reason 109 is representable here but the baseline never
...
--
-- WHY : Assumptions: rejects are a persisted table rather than an error channel
...
CREATE TABLE ledger.transaction_category_balances (
    ...
    -- WHY : Assumptions: BIGINT is required rather than merely chosen.
    ...
    -- WHY : Assumptions: the same two-character code as on the transaction
    ...
    -- WHY : Assumptions: CHAR(4) for the same code-versus-quantity reason as on
    ...
    -- WHY : Assumptions: exact fixed point at precision 11, scale 2, from
    ...
    -- WHY : Assumptions: a three-part composite key with exactly this arity and
...
-- WHY : Alternatives Considered: the composite natural key above is the sole
