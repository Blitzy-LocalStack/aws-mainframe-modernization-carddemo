...
| `consistency` | that each gate the standard calls live is present in the **active configuration** of the file the standard names for it — parsed as XML, JSON, TOML or Python where the standard library has a parser, comment-stripped per language where it does not | a claim whose mechanism has been deleted, renamed, disabled, or never wired — including one whose mechanism is gone while the comment describing it survives |
| `self-test` | that the two detectors still fire and still stay silent, that the governed-format decision still admits every format the checks must read, and that each extractor reads active configuration rather than prose about it | a detector that has stopped detecting, a format that has silently dropped out of scope, or an extractor relaxed back to a text search — each of which would otherwise report a vacuous pass |
...
* **JSON, for the `what` check only.** Strict JSON has no comment lines, so every
  leading header block a JSON file could have is zero lines long and the
  header-relative rule has nothing to be relative to. The `labels` check still reads
  `.json`, because a label's written form does not depend on where a header ends.
* **`ui/package-lock.json`.** Generated by `npm ci`, roughly twenty thousand lines of
  resolved versions and integrity hashes, and no authored prose. It is excluded by
  exact path rather than by suffix so that its authored sibling `ui/package.json` —
  whose `carddemoDependencyRationale` block is the reason `.json` is governed at all —
  stays in scope.
* **The fixed-width record fixtures under `services/*/src/test/resources/fixtures/`.**
  Their `.txt` extension is incidental: the bytes are a copybook record layout, not
  prose. Excluded by path SEGMENT rather than by naming the three
  `data-migration/requirements*.txt` manifests that are governed, because an
  enumeration would leave a fourth manifest silently ungoverned while the gate still
  reported coverage.
* **Python packaging metadata directories** — anything named `*.egg-info`, matched by
  suffix because the name is derived from the distribution and no fixed name excludes
  it.
