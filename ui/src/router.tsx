...
/*
 * Trade-offs: the account-update screen, the three card screens and the transaction-capture screen are
 * loaded LAZILY and sign-on is not, and the split is the decision rather than the mechanism. Eager
 * imports would be simpler -- no `lazy`, no `Suspense`, no default-export adapter below -- and would
 * put all six screens plus their antd dependencies into the single chunk the browser must fetch before
 * the sign-on form appears.
 * Sign-on is the only screen an unauthenticated operator can reach, and the five guarded screens pull
 * the heaviest components in the tree (`Table`, `Descriptions`, `Form`, `Select` and `Popconfirm`, with
 * account update alone painting 43 data controls), so an eager bundle makes every operator pay for the
 * browse, detail, card-update, account-update and transaction-capture screens in order to see a login
 * form. The cost
 * accepted is real and is paid at a different moment: the FIRST navigation to each guarded route
 ...
 * instead of having to be searched for.
 */
    ...
    <BrowserRouter>
      <Suspense fallback={<Spin size="large" />}>
        <Routes>
          ...
          {/*
           * Assumptions: `/account/update` is declared as a STATIC path with no parameter and with no
           * admin guard. The account identifier is entered on the screen itself rather than carried
           * in the URL, mirroring `COACTUPC`, whose only unprotected control on first entry is the
           * `ACCTSID` search key; and `app/cpy/COMEN02Y.cpy` L35 gives option 2 the user type `'U'`,
           * so gating it on the `carddemo-admin` claim would lock out exactly the operators the
           * option exists for. The card routes need their own selector module because they carry an
           * opaque identifier in the path, and this route needs no such helper because it carries
           * nothing. `ui/nginx.conf` already names this path in its SPA history fallback, so the
           * served application and this table agree.
           */}
          <Route path="/account/update" element={guarded(<AccountUpdateScreen />)} />
          ...
          {/*
           * Assumptions: the path is spelled here as the migration plan's screen inventory spells it, and
           * it is a literal rather than an imported constant because no selector guard validates it -- the
           * capture screen takes no route parameter, so there is nothing to seal. The card routes import
           * their paths from `./routes/cards` precisely because theirs carry an opaque selector that has
           * to be validated in one place.
           */}
          <Route path="/transactions/new" element={guarded(<TransactionAddScreen />)} />
