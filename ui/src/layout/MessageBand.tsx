...
export function MessageBand({
   ...
   * the DOM and is announced in full, while the three overflow declarations
   * confine it to the single reserved line. Slicing the value at 75 was the
   * alternative and was rejected: the display region is 78 or 80 characters
   * wide depending on the mapset, so cutting the string at the 75-character
   * content contract would discard characters the baseline itself can render,
   * and it would discard them irreversibly rather than merely off-screen. What
   * is given up is that a clipped tail is not visible at a glance, which the
   * hover title below mitigates.
   *
  ...
  const messageTextStyle: CSSProperties = {
    color: token[SEVERITY_COLOR_TOKENS[severity]],
    fontWeight: token[TYPOGRAPHY_TOKENS.brightEmphasis],
    display: "block",
    overflow: "hidden",
  ...
  /*
   * Trade-offs: the hover title is attached only when the text exceeds the
   * content contract, not on every message. A `title` on an element that
   * already shows the same text gives it a redundant accessible name, and
   * paying that cost on all 21 screens to cover the exceptional case is the
   * wrong trade — every catalogued message fits the contract by construction,
   * so for those the band is already wide enough and a tooltip would reveal
   * nothing. Above the contract the reveal is the only way to read the tail,
   * and the redundancy is worth it.
   *
   * Assumptions: the attribute is applied by spreading an object that either
   * holds it or is empty, rather than by passing `undefined` on the short path.
   * `exactOptionalPropertyTypes` is enabled and antd's text props declare
   * `title?: string` without `undefined`, so an explicit `undefined` is a type
   * error there — under that setting an absent property and an `undefined` one
   * are genuinely different things, and only absence is accepted.
   */
  const hoverTitleProps: { title?: string } =
    text.length > MESSAGE_BAND_CONTENT_WIDTH ? { title: text } : {};
  ...
  return (
    <Flex align="center" data-testid={MESSAGE_BAND_TEST_ID} style={bandStyle}>
      ...
      <Alert
        ...
        style={ALERT_STYLE}
        title={
          <Typography.Text style={messageTextStyle} {...hoverTitleProps}>
            {text}
          </Typography.Text>
        }
        type={severity}
