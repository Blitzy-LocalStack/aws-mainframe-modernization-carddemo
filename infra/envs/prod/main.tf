...
#   by the four services that publish a paged list; the messaging key is held by
#   the authorization consumer alone and the internal-identity key by exactly
#   three services. Sharing one value would mean a holder of any one capability
#   could exercise the others -- a service able to seal a cursor could mint an
#   internal bearer token -- and rotating any one purpose would invalidate all
#   three at once.
#   Trade-offs: three secrets cost three entries to provision and three attended
...
#   keys is only operational.

ephemeral "random_password" "pagination_cursor" {
  # WHY : Assumptions: EPHEMERAL rather than a managed random_password, for the same
  #       reason as the two keys above -- a managed resource retains its result in
  ...
  #       as the two keys above do. Rejected because it would only APPEAR to work: with
  ...
  #       start on all four consuming services at once. Encoding explicitly makes the
  ...
  #       through four task definitions, so a shell-quoting accident on any operator
...
resource "aws_secretsmanager_secret" "pagination_cursor" {
  ...
  description = "Purpose-scoped symmetric key the CardDemo services seal and open keyset pagination cursors with in the ${var.environment} environment, read by com.carddemo.common.web.CursorToken. Generated by this root and injected as a scalar secret into exactly the four services that publish a paged list. Distinct from the internal-identity signing key, the messaging HMAC key and the data-migration masking key."
...
resource "aws_secretsmanager_secret_version" "pagination_cursor" {
  ...
  #       is consumed through a STRICT base64 decoder, unlike the two keys above which
  #       accept raw text. Storing the characters unencoded would make the stored value
  ...
  #       four services, without any configuration having changed. Advancing it is the
...
locals {
  ...
  #       exactly the ARN each container reads -- the same shape as
  #       internal_identity_secret_sources above and deliberately not the composite
