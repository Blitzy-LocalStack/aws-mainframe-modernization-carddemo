...
locals {
  ...
  #       one.
  # WHY : Assumptions: v0.49.0 is the highest semantic version the upstream
  #       repository publishes; `latest` is the only non-semver tag it carries.
  #       Running a superseded collector is a supply-chain position rather than a
  #       preference, because a sidecar attached to every workload is the one
...
#       image.
# WHY : Assumptions: the module provisions ONE further repository beyond those ten,
#       and it is not an eleventh deployable. `aws-otel-collector` mirrors a pinned
#       third-party image this repository caches rather than builds, and it is
#       declared in the module's separate third_party_mirror_repository_names input
#       precisely so the ten-deployable count section 0.4.1.6 fixes stays assertable
#       while the cache remains visible. The mirror is required rather than
#       convenient: infra/modules/ecs-service runs the collector sidecar in every
#       task, infra/modules/network enumerates the application tier's egress instead
#       of allowing 0.0.0.0/0, and Amazon ECR Public has neither an interface
#       endpoint nor a managed prefix list -- so without the mirror no task could
#       pull its sidecar and none would start. docs/runbooks/deploy.md carries the
#       commands that populate it.
...
#       write-only shape as the four other generated keys in this root -- the two
#       per-caller internal-identity keys, the pagination-cursor key and the
#       reporting-artifact key: an ephemeral generator whose result reaches only
#       secret_string_wo, so the value is absent from state. Both of these
#       paragraphs pointed at "the messaging key below" instead, and there is no such
#       key below or anywhere else in this root -- the note beneath this block records
#       its withdrawal -- so the shape was compared against nothing and the separation
#       was claimed from a key that does not exist.
# WHY : Assumptions: a SEPARATE secret from the internal-identity keys, rather than
#       one key reused. Only card-service holds this one, and the
...
resource "aws_secretsmanager_secret" "card_selector" {
  #checkov:skip=CKV2_AWS_57:A selector minted under one key cannot be opened under another, and services/card-service/src/main/resources/application.yml records why that matters here: a selector is a card row's stable address and must keep opening for as long as a client might hold one, unlike a pagination cursor, which names a position in one browse and is meant to expire. An unattended rotation function would therefore invalidate every selector already issued, and each single-card route a client reached from a list it still has on screen would stop resolving. Rotation is an attended procedure that reissues selectors with the deployment, documented in docs/runbooks/deploy.md, rather than an automatic one. The four sibling key secrets in this root -- the two per-caller internal-identity signing keys, the pagination cursor key and the reporting-artifact key -- each carry the same exception for the same class of reason, every one naming the runbook section that performs its own rotation. This sentence has been short three times: it said TWO siblings, then FOUR, then FIVE by continuing to count a messaging HMAC key that the withdrawal note above this resource records as removed with the component that read it. The siblings are enumerated rather than counted so that the list and the resources it describes can be compared directly, which is what a bare number defeated.
...
#   generates FIVE key secrets and the holder set of each is stated once here, so a
...
#   internal bearer token -- and rotating any one purpose would invalidate all five
#   at the same moment.
...
#   ⚠️ Refactoring Rationale: this block previously carried three counts that
...
#   than none because it reads as corroboration.
#   The enumeration that replaced those counts then went stale in its own way, which
#   is the correction now applied: it kept a MESSAGING HMAC KEY as its first entry,
#   and no such secret exists in this root -- the withdrawal note above the
#   card-selector resource records that the ephemeral block, the secret, its
#   write-only version and the task injection were all removed together when the
#   component that read the property was deleted. So the enumeration named six keys
#   where five are declared, and enumerating rather than counting did not prevent it:
#   a list is only self-checking if a REMOVAL is applied to it as well as an
#   addition, and this repository had five ephemeral blocks and five secret resources
#   in this file the whole time. Those ten resources are what the list is checkable
#   against, and the count in the heading above is now stated as the length of the
#   list immediately below it rather than as an independent figure.

ephemeral "random_password" "pagination_cursor" {
  # WHY : Assumptions: EPHEMERAL rather than a managed random_password, for the same
  #       reason as the three keys declared above it -- the card-selector key and the
  #       two per-caller internal-identity keys -- every one
...
#   two, and the card-selector key by one.
...
#   exercise the others, and rotating one purpose would invalidate all five at once
#   -- which here means every statement object already written becomes unlocatable
#   by the application's own lookup.
#   ⚠️ Refactoring Rationale: this paragraph called it "a FOURTH generated key",
#   said the internal-identity key was held "by exactly three services" and that
#   rotation would invalidate "all four at once"; the correction to those then said
#   "all six" and listed a messaging key among the holder sets. Every version was
#   stale for the same structural reason: the single internal-identity key was split
#   per caller into two, two further keys were added, and one -- the messaging HMAC
#   key -- was WITHDRAWN, and only the withdrawal was missed here because a
...
locals {
  ...
  special_runtime_parameters = {
    ...
    # WHY : (1) ⚠️ Assumptions: the FOUR queue entries below carry the queue's URL
    #       and never its NAME, and the two are not interchangeable. Every value is a
    #       module.sqs *_queue_url output and every consuming property is spelled
    #       *-queue-url, so the representation the property name promises is the
    #       representation it receives. Three statements that stood here are withdrawn.
    #       The count was SIX, from a time when reference-service consumed the date
    #       flow from a queue; that module now consumes no queue at all and declares no
    #       queue key, so its three entries are gone and the batch error-sink entry is
    #       the fourth beside account's three. The second was a named consumer,
    #       com.carddemo.reference.service.DateInquiryMessageListener, which does not
    #       exist -- the date conversion is reached synchronously through
    #       DateConversionController over DateConversionService, and
    #       infra/modules/ecs-service already records the listener as retired. The
    #       third was that "the consumers accept EITHER representation", each
    #       recording the decision "at its own queueUrl method": there is one consumer,
    #       com.carddemo.account.service.InquiryMessageListener, it has no such method,
    #       and it admits a URL ONLY -- com.carddemo.common.messaging.QueueDestination
    #       .requireQueueUrl refuses a bare name at construction so a misconfigured
    #       destination stops the service instead of failing on the reply path after a
    ...
    #       name.
    #       (3) ⚠️ Trade-offs: URLs are the only accepted representation rather than
    #       one of two, and the reason is that an address is the value the caller
    #       actually needs. A name obliges the consumer to spend a GetQueueUrl call at
    #       start-up, on a queue this root has already resolved, and to hold the IAM
    #       grant for it; an address needs neither. What is given up is a local
    #       configuration that names its queue, and that is accepted because the
    #       consumer refuses one outright at construction -- so the alternative was
    #       never available to be kept. This item previously reconciled a mismatch
    #       between property names and values by pointing at consumers that accepted
    #       both; the properties are spelled *-queue-url, so there is no mismatch left
    #       to reconcile. The GetQueueUrl grant that stood further down for the
    #       hypothetical NAME path is withdrawn with the claim: a grant covering a call
    #       no code can make is privilege with no purpose, which least privilege
  ...
  #       "platform endpoints" is too vague to check against the map below. Three name the
  #       Aurora cluster a workload connects to -- `aurora/host`, `aurora/port` and
  #       `aurora/database`; one names the dataset bucket every generation-writing state
  #       and the ETL address objects in -- `datasets/bucket`; and one carries the seeded
  #       identity subjects the ETL cannot derive from the records it loads --
  #       `identity/seed-user-subjects`. There is no state-machine entry and no queue
  #       entry: the reporting context receives its machine ARN as a container
  #       environment variable from local.special_service_environment above, and every
  #       queue address arrives the same way.
  ...
  #       .github/workflows/infra-ci.yml, which counts this map's entries and fails the
  #       build when the two disagree. A count nothing measures is what drifted before --
  #       the sentence on the publishing resource below went on describing state-machine
  #       ARNs after they were removed from this map, and nothing in the package could
...
#       service would publish the same value under seven names and leave a reader unable
#       to tell which copy is authoritative.
# WHY : ⚠️ Refactoring Rationale: this sentence also listed "the batch state-machine
#       ARNs" among the keys, and no such key is in the map. The four that were there
#       were withdrawn -- the argument sits on local.platform_parameters above -- and this
#       description was left describing them, which is the worst state for an inventory
#       sentence to be in: a reader sizing the grant a task needs would have concluded
#       that an orchestration handle is discoverable from Parameter Store, and no grant or
#       code path supports that. The five keys the map actually holds are enumerated at
#       the local, not restated here, so there is one inventory rather than two.
...
# WHY : ⚠️ Assumptions: the three actions are the whole of what a consumer needs and
#       sqs:GetQueueUrl is deliberately not among them -- but not for the reason stated
#       here, which was the exact inverse of the truth. It said the listener resolves
#       each destination "by NAME with GetQueueUrl" and that "the roots publish the
#       queue NAME rather than the URL for exactly that reason"; this root publishes
#       URLs from module.sqs *_queue_url outputs, the listener admits a URL only, and
#       the container uses a value carrying an http or https scheme verbatim rather
#       than looking it up. So GetQueueUrl is absent because NOTHING calls it, which is
#       a stronger reason than the one it replaces, and receive, delete and
...
data "aws_iam_policy_document" "account_runtime" {
  ...
  # WHY : ⚠️ Refactoring Rationale: a ResolveAccountInquiryDestinationsByName
  #       statement granting sqs:GetQueueUrl on the reply and error ARNs stood here and
  #       is WITHDRAWN. It was added to cover a name-resolution path, on the stated
  #       ground that "the consumers now accept either representation directly" and
  #       that this statement covered the NAME half so a local or future configuration
  #       supplying a name would be "authorised rather than merely expressible". No
  #       such path exists: this root publishes URLs, and the consumer's
  #       QueueDestination.requireQueueUrl refuses a bare name at construction, so a
  #       configuration supplying a name never reaches a lookup -- it stops the service
  #       instead. A grant that authorises a call no code can make cannot fail closed,
  #       and it cannot be exercised either; what it can do is let this role discover
  #       the address of a queue it may not otherwise name, which is precisely what
  #       least privilege withholds. If the consumer is ever widened to admit a name,
  #       the grant returns with it in the same change.
  # WHY : Assumptions: the request queue needed no entry of its own even then. The
  #       listener container is handed that queue's URL and uses it verbatim, so it
  #       calls GetQueueUrl for nothing, and GetQueueAttributes on it is already
  #       granted above.
...
module "ecs_service" {
  ...
  #       back to its tag. A tag identifies an artifact by a label the registry lets a
  #       push move, whereas the digest identifies the bytes; the nine task images
  #       already resolve this way through image_digests, and the sidecar -- the one
  #       container every workload runs -- should not be the exception. The fallback is
  #       what lets a plan run before any digest has been recorded, and repository
  #       immutability makes the tag form safe as that fallback, so this is defence in
  #       depth rather than a correction of something broken.
  ...
  # WHY : Trade-offs: development retains a HIGHER proportion of successful traces
  #       than production -- 25 against the module's default of 5 -- because this
  #       environment's whole traffic is a test run, so a five-percent sample of a
  #       handful of requests routinely retains none of the successful path an engineer
  #       is trying to read. Error traces are kept in full in both environments by the
  #       collector's separate always-keep policy, so this value only ever changes how
  #       much of the working path is visible, never whether a failure is.
  telemetry_success_sample_percentage = 25
