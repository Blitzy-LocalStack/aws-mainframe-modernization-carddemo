...
#   - Trade-offs: the CLI constraint is a floor while the provider constraint is
...
#   - Assumptions: the calling root configures `provider "aws"` -- the region and
...
terraform {
  ...
  # here could only be raised in lockstep across every module under
  ...
  # Assumptions: the AWS provider is CONFIGURED by the calling root
  ...
  #
  # Assumptions: state is a root-level concern too, so no `backend` block appears
  ...
  required_providers {
    ...
    #
    # Trade-offs: `~> 6.56` accepts 6.56.x patches and later 6.x minors but
