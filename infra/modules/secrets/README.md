...
| <a name="input_name_prefix"></a> [name\_prefix](#input\_name\_prefix) | First segment of every secret name this module composes, so that all of<br/>this stack's secrets sort together and are addressable by one IAM resource<br/>pattern. Also the value sibling modules use as the leading component of<br/>their resource names, which is why the accepted charset is narrower than<br/>Secrets Manager alone would require. THIS module imposes 2 to 32<br/>characters of lower-case letters, digits and single interior hyphens,<br/>beginning with a letter; the environment roots forward one prefix to<br/>sixteen modules and so enforce the narrower INTERSECTION of all of them. | `string` | `"carddemo"` | no |
