...
#   - Assumption: this file is parsed as part of a MODULE, never a root. Its
...
#   - Trade-off: every variable that is a name segment carries both a charset
...
variable "name_prefix" {
  ...
  # WHY : Trade-off -- the ceiling is 32 rather than the 512 characters a
...
# WHY : Assumption -- this module has an `environment` variable even though
...
variable "environment" {
  ...
  # WHY : Trade-off -- the 16-character ceiling is a name-segment budget for the
...
# WHY : Assumption -- there is deliberately NO default. The only two candidate
...
variable "kms_key_arn" {
  ...
  # WHY : Trade-off -- this asserts the ARN's SHAPE, not its identity. It
...
# WHY : Trade-off -- marking such an input `sensitive = true` was also
...
# WHY : Assumption -- a username is an identifier and a password is a
...
variable "database_master_username" {
  ...
  # WHY : Trade-off -- the ceiling is 16, the NARROWER of two figures AWS
...
variable "password_length" {
  ...
  # WHY : Trade-off -- the two bounds have different natures and only one is a
...
# WHY : Alternatives Considered -- defaulting this to the eight known service
#       role names was considered and rejected. The module would then hold a
#       second copy of the service inventory, and a copy is exactly what drifts:
#       adding a ninth bounded context would leave this default stale with
#       nothing detecting the disagreement, because a stale default still
#       applies cleanly. Defaulting to the empty set makes the calling root the
#       single place the inventory is stated, alongside the schema and role
#       definitions the ETL creates. The set contains only NAMES, so it is
#       non-secret and belongs in a committed tfvars file like every other
#       value in this module.
# WHY : Assumption -- the empty default is also the safe one. A root that has
#       not yet been wired up creates the master credential and no service
#       credentials, which is a coherent partial stack; a non-empty default
#       would instead create credentials nothing consumes, and an unused
#       credential is harder to notice than a missing one.
variable "service_credential_names" {
  description = <<-EOT
    Names of the per-service database roles that each need their own generated
    credential; one secret is created per element. Expected to be the roles
    owning one schema per bounded context -- auth, account, card, ledger,
    reference, batch and authorization -- plus the read-only role the reporting
    service uses, matching the schemas and roles the data-migration bootstrap
    SQL creates. Defaults to empty so the calling environment root is the only
    place this inventory is stated.
  EOT
  type        = set(string)
  default     = []

  # WHY : Assumption -- each element becomes BOTH a PostgreSQL role name and a
  #       segment of a secret name, so it is validated against the stricter of
  #       the two: a lowercase identifier. Accepting mixed case would be the
  #       subtler failure, because PostgreSQL folds an unquoted identifier to
  #       lowercase while a secret name preserves it, so "Ledger" would create a
  #       secret that no longer matches the role it is named for.
  validation {
    condition = alltrue([
      for role_name in var.service_credential_names :
      can(regex("^[a-z][a-z0-9_]*$", role_name)) && length(role_name) <= 63
    ])
    error_message = "Each service_credential_names element must be lowercase letters, digits and underscores, start with a letter, and be at most 63 characters -- for example \"ledger\"."
  }
}
...
# WHY : Trade-off -- this is the sharpest trade-off in the module, and both
...
# WHY : Assumption -- the default is the PROTECTED value, not the convenient
...
# Rotation
#
# WHY : Assumption -- rotation is OFF by default and nothing in this stack
...
variable "rotation_lambda_arn" {
  ...
  # WHY : Trade-off -- same shape-not-identity limit as kms_key_arn above: this
...
variable "rotation_automatically_after_days" {
  ...
  # WHY : Trade-off -- this rule reads the OTHER rotation variable, which a
...
# WHY : Trade-off -- tags arrive as an input and main.tf applies them to each
