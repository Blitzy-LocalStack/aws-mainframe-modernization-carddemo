...
terraform {
  # WHAT: bound the CLI from below at 1.15.0 instead of pinning one release.
  ...
  required_providers {
    # WHAT: admit hashicorp/aws 6.56 and later 6.x releases, and no 7.x.
    ...
    # WHAT: no `hashicorp/random` entry, though four directories in this
    #       package do pin that provider at `~> 3.9`.
...
# WHAT: no `provider "aws"` block, in this file or anywhere in this module.
...
# WHAT: the remaining absences, listed together rather than one per topic.
...
#       - No `.terraform.lock.hcl`, and none is expected to appear. `init`
#         writes the lock at the ROOT that resolves the provider graph, which
#         is why the three tracked lock files live in infra/bootstrap,
#         infra/envs/dev and infra/envs/prod. One here would record a
#         resolution nothing performs and could contradict the roots'.
# =============================================================================
