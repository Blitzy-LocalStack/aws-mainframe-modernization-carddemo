...
This README is the prose half of Rule 1 Explainability required by AAP section
0.2.1.6. The mechanical half is enforced by
[TFLint](../../.tflint.hcl) and the
[terraform-docs configuration](../../.terraform-docs.yml), while the required
HCL form is summarised by the
...
**Assumption:** `signing_behavior = "always"` keeps control of origin
...
**Assumption:** both custom error responses set
...
The navigation model is client-side by design. A mainframe transfer such as
the `EXEC CICS XCTL` at `app/cbl/COSGN00C.cbl:L245` becomes a `navigate()`
decision selected from the JWT group claim. The edge rewrite is what lets that
...
**Assumption:** the lineage files are specifications, not edit targets. The
...
**Trade-off:** encryption is intentionally asymmetric. SPA objects use the
...
**Assumption:** `s3_kms_key_arn` is necessary but not sufficient. The key
...
**Assumption:** the ACM certificate is mandatory and must be issued in
...
**Assumption:** `s3_kms_key_policy_id` is an ordering token, not a credential.
...
**Assumption:** `api_connect_src_origins` is the only cross-origin API list
...
**Assumption:** terraform-docs renders output descriptions directly from
...
**Trade-off:** the default price class restricts the edge-location set to
...
2. **The response-headers policy is module-owned.** It sets a content-security
   ...
   **Trade-off:** `style-src 'unsafe-inline'` is retained because the Ant Design
...
3. **Only `GET` and `HEAD` reach the origin.** **Assumption:** the origin is a
...
7. **Logging delivery resources use a `us-east-1` regional override.**
   **Assumption:** CloudFront's logging control plane is fixed there even when
...
8. **Both buckets are versioned and lifecycle-bounded.**
   **Trade-off:** versioning preserves a rollback path but accumulates
...
11. **The routing function is published and runs at `viewer-request`.**
    **Assumption:** publication places the function in the LIVE stage referenced
    ...
    create a loop.
12. **The SPA bucket enables an S3 bucket key.** **Trade-off:** grouping
...
13. **IPv6, edge compression, and SNI-only TLS are enabled.**
    **Assumption:** an IPv6-only viewer must be able to reach the same
...
# WHY : CI treats formatting drift as a review failure rather than rewriting it.
terraform fmt -check -recursive infra/

# WHAT: initialise and validate all Terraform roots without remote state.
# WHY : environment-root validation resolves this module with real wiring, and
...
# WHY : this isolates an invalid resource argument from root-level wiring errors.
...
# WHY : the absolute config path prevents recursive lint from falling open.
...
# WHY : CI is check-only; it never hides contract drift by rewriting this file.
...
| [Code documentation standard](../../../docs/CODE_DOCUMENTATION_STANDARD.md) | Rule 1's HCL analogue and the prose/mechanical documentation split |
