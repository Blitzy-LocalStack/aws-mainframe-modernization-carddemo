...
#
# Parameters:
#   Twenty-seven inputs, each declared below with an explicit `type` and a
...
#                               kms_key_arn, secrets_kms_key_arn
...
#     is a tfvars file or a CI input. The master credential is instead generated
#     INSIDE AWS by RDS -- main.tf sets manage_master_user_password -- and this
#     module never sees it: it supplies only the key that encrypts it,
#     secrets_kms_key_arn, and publishes only the ARN of the secret RDS created.
#     A key ARN and a secret ARN each name where something lives and confer no
#     access to it, which is why both are legitimate inputs or outputs while a
#     password would be neither.
...
variable "secrets_kms_key_arn" {
  description = <<-EOT
    ARN of the customer-managed KMS key that encrypts the master credential RDS
    generates and manages for this cluster, supplied by the kms module. This is
    the SECRETS key, deliberately not the same key as kms_key_arn, which
    encrypts the data. Required with no default, because falling back to the
    AWS-managed secretsmanager key would give up the key policy that separates
    who may read a credential from who may read the data.
  EOT
  type        = string
  nullable    = false

  # WHY : Refactoring Rationale: this input REPLACES a
  #       `master_credential_secret_arn` that named a secret generated by the
  #       sibling secrets module and was never consumed for anything but a
  #       cross-ARN precondition. `manage_master_user_password` in main.tf makes
  #       RDS create and populate a secret of its own, so two authorities existed
  #       for one credential: the cluster authenticated against the RDS-managed
  #       value while an operator following the input contract would have read a
  #       different, unrelated secret -- a break-glass path that silently does not
  #       open. The duplicate secret is removed from infra/modules/secrets and the
  #       unused input is removed here, leaving RDS as the single owner. What the
  #       module still needs from a sibling is not a secret but a KEY, which is
  #       what this input supplies.
  # WHY : Assumptions: a KMS key ARN is not itself sensitive -- it names a key and
  #       confers no use of it, which requires a grant in that key's own policy --
  #       so it is not marked sensitive, and a reviewer can see in plan output
  #       which key is being wired in.
  # WHY : Assumptions: the shape check accepts any AWS partition, for the same
  #       reason kms_key_arn's does: a partition-specific pattern would break the
  #       module in GovCloud and China regions for no benefit here. The stronger
  #       property -- that this key and the data key name the same partition,
  #       region and account -- compares two variables and is therefore enforced
  #       by a `precondition` on the cluster in main.tf rather than here.
  validation {
    condition     = can(regex("^arn:aws[a-zA-Z-]*:kms:[a-z0-9-]+:[0-9]+:key/.+$", var.secrets_kms_key_arn))
    error_message = "The secrets_kms_key_arn must be a KMS key ARN of the form arn:aws:kms:<region>:<account-id>:key/<key-id>. Pass the secrets key from infra/modules/kms, not the Aurora data key: the two exist so that reading the master credential and reading the encrypted data are separately grantable."
  }
}
...
variable "master_username" {
  description = <<-EOT
    ...
    RDS reserves. The matching password is NOT an input to this module: RDS
    generates it, stores it in a secret of its own encrypted with
    secrets_kms_key_arn, and this module publishes only that secret's ARN as
    master_user_secret_arn. A user name is not a secret, so this input is not
    marked sensitive and does carry a default.
  EOT
  type        = string
  nullable    = false
...
variable "port" {
  description = <<-EOT
    ...
    makes the cluster unreachable, so the environment root passes the network
    module's database_port here rather than a second literal.
  EOT
  type        = number
  nullable    = false
  default     = 5432
  ...
  # WHY : Refactoring Rationale: the coupling is now expressible rather than only
  #       describable. infra/modules/network/variables.tf declares database_port
  #       as the single settable source of that port, and the environment root
  #       wires this input to it -- `port = module.network.database_port` -- so
  #       the rule and the cluster it protects are supplied from one assignment.
  #       For a period that input did not exist in the network module, which left
  #       this port freely settable against a rule no root could change with it;
  #       it was restored for that reason. Assumptions: the network input applies
  #       the SAME 1150-65535 bound this validation applies -- narrowed there to
  #       whole numbers, which this block does not check -- and matching the bound
  #       is load-bearing in one direction specifically. The network input
  #       previously admitted from 1024, so 1024-1149 was a window in which a
  #       value passed there and was refused HERE, failing the plan with a message
  #       about the range RDS accepts and naming only this cluster. The remaining
  #       asymmetry runs the other way, the network input being the stricter, which
  #       cannot produce that misdirected diagnostic.
  validation {
    condition     = var.port >= 1150 && var.port <= 65535
    error_message = "The port must be between 1150 and 65535, the range RDS accepts for a PostgreSQL cluster and the same range infra/modules/network/variables.tf accepts for database_port."
  }
}
...
variable "enable_http_endpoint" {
  description = "Whether the Aurora Data API endpoint is enabled. Fixed true because Terraform-invoked schema bootstrap, credential rotation and the AnalyzeTables batch state all use rds-data without a VPC-bound PostgreSQL driver."
  type        = bool
  nullable    = false
  default     = true

  # WHY : Refactoring Rationale: exposing a false value only creates a
  #       deployment that plans successfully and then fails every database
  #       administration Lambda. The input remains visible so an attempted
  #       weakening fails with the reason at the calling root.
  validation {
    condition     = var.enable_http_endpoint
    error_message = "enable_http_endpoint must remain true because bootstrap, credential rotation and table analysis use the Aurora Data API."
  }
}
...
# -----------------------------------------------------------------------------
# WHY : Assumptions: the ETL and the services resolve connection coordinates from
#       Parameter Store rather than from a Terraform output, so the namespace is
#       a contract shared with carddemo_migration.config -- changing it here
#       breaks a consumer that this module cannot see. It is an input rather than
#       a literal only so a second deployment in one account can hold two
#       namespaces; the default is the value the Python resolver expects.
#       Alternatives Considered: publishing the endpoint only as a Terraform
#       output. Rejected because a consumer outside Terraform -- the ETL image,
#       an operator command in a runbook -- cannot read another root's state, and
#       giving it one would mean granting state access to read a hostname.
variable "parameter_prefix" {
  description = <<-EOT
    Canonical Parameter Store namespace under which the host, port and database
    connection parameters are published. Must begin with one slash and carry no
    trailing slash; the default matches carddemo_migration.config.
  EOT
  type        = string
  nullable    = false
  default     = "/carddemo"
  ...
  validation {
    ...
    error_message = "parameter_prefix must begin with one slash, contain non-empty path segments of letters, digits, period, underscore or hyphen, and carry no trailing slash."
  }
}
