...
#   - Assumption: every variable declared here is read by
...
#   - Trade-off: the defaults carry application knowledge -- the ten
...
#   - Assumption: every block below states `nullable` explicitly, and that is
...
# WHY : Assumption: nine Maven modules under services/ yield only EIGHT
...
variable "repository_names" {
  ...
  # WHY : Trade-off: shipping the ten names as a default rather than demanding
  ...
  # WHY : Assumption: `nullable = false` makes an explicit null resolve to the
  ...
  # WHY : Trade-off: a few lines of HCL for a readable `plan`-time failure. An
  ...
  # WHY : Assumption: the registry rejects a malformed repository name rather
...
# WHY : Assumption: this prefix, not the account itself, is what keeps
...
variable "name_prefix" {
  ...
  # WHY : Trade-off: a default makes the module usable from a root that
  ...
  # WHY : Assumption: this segment is concatenated straight into a registry
...
#       Assumption: no account identifier belongs in the composed name. The
...
variable "environment" {
  ...
  # WHY : Assumption: because this input has no default, `nullable = false`
  ...
  # WHY : Assumption: these two are the only environment roots this package
...
#       Assumption: when a value is supplied it arrives as the `kms` module's
...
#       Trade-off: null is a meaningful value rather than an error, so the
...
variable "kms_key_arn" {
  ...
  # WHY : Assumption: this is the one input in the file where null is a CHOICE
...
#       Trade-off: a failed push cannot be retried under the same tag, so the
...
variable "image_tag_mutability" {
  ...
  # WHY : Assumption: the default is the decision, because neither environment
  ...
  # WHY : Assumption: `nullable = false` makes an explicit null resolve to
  ...
  # WHY : Assumption: the registry takes this value uppercase and rejects any
...
# WHY : Assumption: the registry itself performs the vulnerability scan,
...
#       Trade-off: the variable exists only so a caller can switch scanning
...
variable "scan_on_push" {
  ...
  # WHY : Assumption: the infrastructure pipeline's policy scan asserts that
  ...
  # WHY : Assumption: `nullable = false` makes an explicit null resolve to true
...
# WHY : Assumption: the deployment workflow tags every image with its commit
...
#       Trade-off: the bound is what a rollback can reach back through, so it
...
variable "max_image_count" {
  ...
  # WHY : Trade-off: thirty retains far more revisions than any realistic
  ...
  # WHY : Assumption: the registry requires a whole number of at least one
...
# WHY : Assumption: an untagged manifest is unreachable, not merely old, and
...
#       Trade-off: a nonzero window rather than immediate deletion. Expiring
...
variable "untagged_image_expiry_days" {
  ...
  # WHY : Trade-off: fourteen days outlasts any single aborted-push
  ...
  # WHY : Assumption: the registry counts this in whole days and refuses zero,
...
# WHY : Trade-off: clean teardown is an acceptance criterion for this package
...
#       Assumption: this is the same call the state-bucket input in
...
variable "force_delete" {
  ...
  # WHY : Assumption: an operator who genuinely intends to discard images sets
  ...
  # WHY : Trade-off: one line of configuration that changes nothing observable
...
# WHY : Assumption: root-level and module-level tags compose additively rather
...
#       sites stay uniform.
#       Trade-off: an empty default means a reader of main.tf sees a `tags`
...
variable "tags" {
  ...
  # WHY : Assumption: an empty map is the correct neutral value, not a sign of
  ...
  # WHY : Trade-off: as on `force_delete`, one line that changes nothing
