...
behavioural divergence and is registered as `D-SIGNON-EXISTENCE-UNIFORM` in
section 7.4 of the
[divergence register](../../../docs/architecture/cobol-to-service-traceability.md).
That entry also records the consequence of the refusal below: the published
sign-on contract previously described the distinguishable behaviour as available
"under an explicitly selected legacy provider posture", which this module makes
unreachable, so those sentences have been withdrawn from the contract.
...
| <a name="output_credential_secret_name_prefix"></a> [credential\_secret\_name\_prefix](#output\_credential\_secret\_name\_prefix) | Secrets Manager name prefix beneath which one-time credentials for pool accounts are written, as `<name_prefix>-<environment>/cognito`. The auth service composes an entry name from this prefix, a `/runtime-user/` infix and a digest of the account identifier; the calling root passes it to that service and scopes the service's write grant to a wildcard beneath it. It is the same root the seeded initial-password entries use. No secret value is published. |
