...
#
#   Five values, and deliberately no more: the invoke endpoint a client calls,
#   and the four identifiers a caller needs in order to attach something to
#   what this module built. The resources these values are read from are in
#   main.tf, the inputs that configure those resources are in variables.tf,
#   and the toolchain and provider contract is in versions.tf.
#
#   These five are also the mechanism behind a property stated for the whole
...
#   always wants the endpoint, sometimes wants the API id, and rarely wants
#   the remaining three. infra/.terraform-docs.yml re-sorts these rows when it
#   generates this module's README, so ordering for the reader of the HCL
#   costs the generated table nothing.
#
...
#     is still unreadable -- ordinary, and not a defect. `stage_name` is the
#     one exception and does resolve at plan time, for the reason recorded at
#     its own block below.
...
#     the address a browser is handed -- and the four identifiers name
#     resources rather than granting access to them, so holding one confers
#     nothing that the JWT check at the edge and the IAM policies behind it do
#     not still require. Marking the endpoint sensitive would actively break
...
output "api_endpoint_url" {
  ...
  # WHAT: reads the STAGE's `invoke_url`, not the API's `api_endpoint`.
  ...
  #       variables.tf:L1042-L1050 records as prepending that segment to every
...
output "stage_name" {
  ...
  # WHAT: reads the created stage's `name`, not `var.stage_name`.
