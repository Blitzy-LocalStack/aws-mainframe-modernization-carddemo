...
# layer's JUnit report to reports/normalized/integration.xml. Doing it in the trap
# (rather than inline before each `exit`) guarantees it happens on EVERY
# report-writing path -- normal, warn, build-fail, missing-pytest -- from ONE
# registration. carddemo_normalize_junit is a no-op when no raw report exists, so
# the early usage-error/--help exits (before CARDDEMO_INT_REPORT is set) are safe
# under `set -u` via the ${..:-} guard. Normalize runs BEFORE cleanup purely for
# readability; the two touch disjoint trees (reports/ vs the workspace), so order
# is immaterial.
trap 'carddemo_normalize_junit "${CARDDEMO_INT_REPORT:-}" "$CARDDEMO_REPORTS_DIR/normalized/integration.xml"; carddemo_cleanup_workspace' EXIT
...
if command -v pytest >/dev/null 2>&1; then
    ...
    carddemo_write_empty_junit "carddemo-integration" "$CARDDEMO_INT_REPORT" \
        "pytest not available (need 'pytest' or python3 with pytest); integration layer could not run"
