...
| **G7** | Five of the eight hue-mapped colours measure below the WCAG AA minimum for normal text **on the one surface the frame actually paints** | On `colorBgContainer`, the surface `SURFACE_TOKENS.screen` resolves to and `ui/src/layout/AppShell.tsx` paints across all three zones: `COLOR=BLUE` operand **4.104:1**, `COLOR=TURQUOISE` **2.205:1**, `COLOR=GREEN` **2.265:1**, `COLOR=YELLOW` **1.900:1** and `COLOR=RED` **3.268:1**, each below 4.5:1, while `COLOR=NEUTRAL` **6.978:1**, `COLOR=DEFAULT` **16.558:1** and `COLOR=PINK` **16.558:1** clear it. Historical evidence for the abandoned dark chrome `#001529` is retained: the blue operand measured **4.491:1** there against a 4.5:1 requirement. | **Resolved per ROLE against one uniform surface, in `BMS_TEXT_COLOR_TOKENS`, with the per-role ratio and the in-family shade it beat recorded in `BMS_TEXT_CONTRAST_AUDIT`.** Blue keeps its hue family at `colorPrimaryTextActive` (**6.159:1**) and red at `colorErrorTextActive` (**4.618:1**); turquoise snaps to `colorTextLabel` (**6.978:1**); green and yellow snap to `colorText` (**16.558:1**). Three whole hue families were measured member by member and contain **no** compliant shade — cyan best **3.549:1**, success best **3.463:1**, warning best **2.867:1** — against primary best **6.159:1** and error best **4.618:1**, which do clear it; that is what makes the two snaps a measured impossibility rather than a preference. **⚠️ Refactoring Rationale:** this row previously described a **per-surface** substitution map, `ACCESSIBLE_TEXT_TOKENS`, resolving blue-on-chrome and blue-on-shell-surface to two different ramp steps. That map has been **withdrawn as superseded and removed from `ui/src/theme/tokens.ts`**, because `AppShell` now paints the header, the body and the legend with one surface specifically so that one measurement per role is the whole answer — so the dark-navy fill the map corrected for is no longer rendered anywhere, and a per-surface lookup would reinstate a distinction the one-surface decision deliberately removed. **Both operands and their counts stay recorded** in `BMS_SOURCE_HISTOGRAM`, and the message band is deliberately unchanged because each severity pairs against its own alert tint with a per-severity icon and ARIA role rather than against the document surface. **Asserted on every run** by `ui/src/theme/textContrast.test.ts` per role, plus the three exhaustive family censuses and the retained dark-chrome evidence in `ui/src/theme/contrast.test.ts`. |
| **G8** | No token scale can express a viewport-relative height | `DFHMDI SIZE=(24,80)` fills the display and fixes the key legend on row 24; **one** value is needed, by **one** element — the document's mount point, `#root` in `ui/index.html` | **Registered in the bridge and declared at the DOCUMENT layer.** `ADDITIVE_LAYOUT_VALUES.viewportMinimumHeight` holds `100dvh` as the single record of the value, and `ui/index.html` declares it on `#root` beside the `body { margin: 0 }` reset, so the frame stays one visible viewport tall and the legend sits at the bottom edge of the display as row 24 did. `dvh` rather than `vh` because a mobile browser's collapsing toolbar makes `vh` describe a viewport taller than the visible one, which would push the legend out of view on exactly the narrow viewports the **G1** reflow exists to serve. **⚠️ Refactoring Rationale:** this row previously said the value was consumed by the application frame in `ui/src/layout/AppShell.tsx`, and cited a `ui/src/router.test.tsx` assertion to that effect. Neither holds. A frame sized to exactly one viewport inside a document keeping the user agent's default `body { margin: 8px }` scrolled permanently by 16px, and no component may reset a margin it does not own — so the sizing and the reset moved together to the document, the shell declares no height at all, and the design system's own `flex: auto` stretches the frame to fill the mount point. **No component in `ui/src` consumes the value**, because no component owns the mount point; the register and the stylesheet are held in agreement by `ui/src/layout/appShell.test.tsx`, which reads `ui/index.html` and asserts the declared `min-height` equals the registered value. A second, duplicate registry of the same value (`LAYOUT_METRICS.frameMinBlockSize`) was removed for the same reason. |
