...
| **Dead consumer — reported through health, no alarm resource of its own** | A listener container that is registered and not running, absent under its expected identifier, missing because no listener registry is present, or running against a request queue that cannot be resolved | — | Is this service's queue consumer alive at all | None to configure: the consumer contributes to `GET /actuator/health`, so a dead consumer turns the endpoint `503`, the target unhealthy, and the two rows at the top of this table breach. `account-service` contributes `InquiryListenerHealth` and `authorization-service` contributes `ContainerCyclingWindowBoundary`, each resolving its container by the identifier its own `@SqsListener(id = …)` declares. ⚠ This row exists because the stale-work row below records the same failure from the other side and cannot catch it: a stopped consumer receives nothing, so it fails nothing, so the dead-letter queue stays empty and its alarm stays OK — and until the consumer contributed to health, the task reported itself fine while every request piled up. The stale-work alarm does eventually breach on message age, which is why it is not redundant: it catches a consumer that is running and not progressing, where this row catches one that is not running at all. ⚠ The fourth condition is listed because the container's own running flag does **not** cover it: a container bound to a non-existent queue starts, registers, reports itself running and then fails every receive, so `account-service` additionally requires its request queue to resolve — once, latching on success, so that a queue-service outage cannot deregister every task of a service whose synchronous paths need no queue |
