...
def record_key(record: str) -> str:
    ...
    require_exact_record_width(record, TRANTYPE_LAYOUT)
...
def iter_ascii_transaction_types(
    ...
    records = iter_ascii_text_records(
        source, TRANTYPE_LAYOUT.reclen, min_data_width=_DATA_REGION_WIDTH, layout=TRANTYPE_LAYOUT
    )
...
def read_ascii_transaction_types(path: pathlib.Path) -> Iterator[DecodedTransactionType]:
    ...
    yield from iter_ascii_transaction_types(iter_seed_lines(path, TRANTYPE_LAYOUT.reclen))
