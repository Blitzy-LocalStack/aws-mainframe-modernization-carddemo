...
def record_key(record: str) -> str:
    ...
    require_exact_record_width(record, CARD_LAYOUT)
...
def iter_ascii_cards(source: str | Iterable[object]) -> Iterator[DecodedCard]:
    ...
    records = iter_ascii_text_records(
        source, CARD_LAYOUT.reclen, min_data_width=_DATA_REGION_WIDTH, layout=CARD_LAYOUT
    )
...
def read_ascii_cards(path: pathlib.Path) -> Iterator[DecodedCard]:
    ...
    yield from iter_ascii_cards(iter_seed_lines(path, CARD_LAYOUT.reclen))
