...
Purpose
-------
Turn the disclosure group reference extract into decoded records the Aurora loaders
...
def record_key(record: str) -> str:
    ...
    if len(record) < DISGROUP_LAYOUT.reclen:
        raise RecordLengthError(
            f"a {DISGROUP_LAYOUT.name} record of {len(record)} characters is shorter than the"
            f" declared {DISGROUP_LAYOUT.reclen}, so its"
            f" {DISGROUP_LAYOUT.key_length}-character key cannot be sliced"
        )
...
def read_ascii_disclosure_groups(path: pathlib.Path) -> Iterator[DecodedDisclosureGroup]:
    ...
    with path.open("r", encoding="latin-1", newline="\n") as handle:
        yield from iter_ascii_disclosure_groups(handle)
