...
def test_every_ascii_reader_refuses_a_named_pipe(
    ...
    """
    # Trade-offs: no timeout guards these cases. A reader edited back to a plain `Path.open`
...
def test_every_ascii_reader_bounds_an_unterminated_line(
    ...
    None
        The assertion is the result.
    """
    # Assumptions: the fixture is a hundred records' worth of digits with no separator, which
...
def test_every_ascii_reader_refuses_a_line_that_stops_inside_its_data(
    ...
    None
        The assertion is the result.
    """
    # Refactoring Rationale: the shared text iterator right-pads a short line, which is what
    ...
    #   the fault.
    # Assumptions: the fixture is all digits rather than spaces, so the refusal cannot be
...
def test_a_line_reaching_the_data_region_is_read_with_its_pad_restored(
    ...
    None
        The assertion is the result.
    """
    # Assumptions: this is the companion of the case above and the two differ by ONE
    #   character,
    ...
    # Trade-offs: the row is refused for a FIELD reason or accepted, and both outcomes pass.
...
def test_the_customer_credit_score_is_an_integer_on_both_corpora(
    ...
    None
        The assertion is the result.
    """
    # Refactoring Rationale: the score used to come out as the three characters the record
    ...
    # Assumptions: both corpora are asserted, and their decoded rows are compared for
...
def test_the_customer_identifiers_keep_their_leading_zeroes(repo_root: pathlib.Path) -> None:
    ...
    None
        The assertion is the result.
    """
    # Assumptions: the two other unsigned display fields of this record are asserted to be
...
def test_the_score_projection_names_exactly_one_field() -> None:
    ...
    None
        The assertion is the result.
    """
    # Assumptions: the assertion is over every field of a decoded record rather than over the
...
def _transaction_dataset(records: tuple[str, ...], destination: pathlib.Path) -> pathlib.Path:
    ...
    None
    """
    # Assumptions: the character images encode to cp037 faithfully because this package's
...
def test_a_missing_transaction_source_is_the_only_absence_either_encoding_accepts(
    ...
    None
        The assertion is the result.
    """
    # Assumptions: this is the tolerance the rest of these cases bound. It must be asserted
...
def test_a_directory_is_not_read_as_an_absent_transaction_source(
    ...
    None
        The assertion is the result.
    """
    # Refactoring Rationale: `Path.is_file()` returns False for a directory, so this input
...
def test_a_named_pipe_is_not_read_as_an_absent_transaction_source(
    ...
    None
        The assertion is the result.
    """
    # Trade-offs: as with the nine, no timeout guards this. A regression HANGS the suite
...
def test_a_dangling_symbolic_link_is_not_read_as_an_absent_transaction_source(
    ...
    None
        The assertion is the result.
    """
    # Assumptions: this is the case that most needs stating, because the operating system
...
def test_an_unreadable_transaction_source_is_not_read_as_absent(
    ...
    None
        The assertion is the result.
    """
    # Assumptions: skipped for a superuser rather than worked around, because a mode bit does
...
def test_an_uninspectable_transaction_path_is_not_read_as_absent(
    ...
    None
        The assertion is the result.
    """
    # Alternatives Considered: the inaccessible case is asserted here through a path whose
...
def test_a_zero_byte_transaction_source_yields_nothing_without_being_called_absent(
    ...
    None
        The assertion is the result.
    """
    # Assumptions: empty and absent produce the same RECORDS and different PRESENCE answers,
...
def test_the_transaction_ascii_path_bounds_an_unterminated_line(tmp_path: pathlib.Path) -> None:
    ...
    None
        The assertion is the result.
    """
    # Assumptions: the bound is asserted through the LOWER-bound wording of the message, which
...
def test_the_transaction_ebcdic_path_refuses_a_dataset_that_does_not_divide(
    ...
    None
        The assertion is the result.
    """
    # Assumptions: the refusal is demanded from the FIRST advance of the iterator rather than
...
def test_the_transaction_reader_reads_its_committed_fixture_in_both_encodings(
    ...
    None
        The assertion is the result.
    """
    # Trade-offs: the positive case is asserted from the committed export fixture rather than
...
def test_the_transaction_presence_predicate_reports_only_genuine_absence(
    ...
    None
        The assertion is the result.
    """
    # Assumptions: the predicate is asserted separately from the two entry points even though
